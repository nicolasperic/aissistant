...
Each MView subscription is declared in `etc/mview.xml`:
...
| Attribute | Purpose |
|---|---|
| `id` | Unique MView ID, referenced by indexer |
...
**Exam focus:** The `status` column values are `idle`, `working`. If a cron job dies mid-index, status can remain `working`, causing the next run to skip — this is a known operational failure mode.
...
Magento ORM fires events
        |
        v
Indexer::executeRow($productId) called synchronously
        |
        v
...
<config xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    ...
    <indexer id="catalog_product_price"
             ...
             class="Magento\Catalog\Model\Indexer\Product\Price"
             shared_index="catalog_product_attribute">

        <title translate="true">Product Price</title>
        <description translate="true">Reindex product prices.</description>

        <!-- This indexer requires catalog_product_attribute to be valid first -->
        <dependencies>
...
| `class` | PHP class implementing `IndexerInterface` | Must implement `executeList()`, `executeFull()`, `executeRow()` |
| `shared_index` | This indexer shares index tables with another | Prevents duplicate full reindexes |
| `dependencies` | Indexers that must complete first | Dependency failures cascade |

### 4.2 The IndexerInterface Contract
...
interface IndexerInterface
{
    ...
    /**
     * Reindex a single entity (used by Update on Save)
     */
    public function reindexRow($id): void;

    /**
     * Reindex a list of entities (used by Update by Schedule changelog processing)
     */
    public function reindexList($ids): void;
}
```

**Exam focus:** `reindexList()` is what the MView cron calls — it receives the array of entity IDs from the changelog. `reindexAll()` is what `bin/magento indexer:reindex` calls.

### 4.3 Dependency Resolution

Dependencies ensure index integrity:

```
catalog_product_attribute (EAV index)
        |
        | must complete first
        v
...
**Exam focus:** If you run `bin/magento indexer:reindex catalog_product_price` but `catalog_product_attribute` is invalid, Magento will reindex the dependency first automatically. This is declarative dependency management — not procedural scripting.
...
class ProductBlock extends Template implements IdentityInterface
{
    ...
    public function getIdentities(): array
    {
        ...
        // Returns: ['cat_p', 'cat_p_42'] for product ID 42
    }
}
```
...
```php
// Product model cache tags:
['cat_p', 'cat_p_42']           // All product pages + product 42 pages

// Category model cache tags:
['cat_c', 'cat_c_5']            // All category pages + category 5 pages

// CMS Page cache tags:
['cms_p', 'cms_p_3']            // All CMS pages + page ID 3

// Custom tag (always use constants):
...
X-Magento-Tags: cat_p,cat_p_42,cat_c,cat_c_5,store,cms_p_1
        |
        v
...
public function getIdentities(): array
{
    ...
    // This only invalides when ANY product changes - too broad
}

// CORRECT - returns both general and specific tags:
public function getIdentities(): array
{
    return [
    ...
    // ['cat_p', 'cat_p_42'] - invalidates on product 42 save specifically
}
```
...
The synchronization uses a **hash-based versioning** approach:

```
Write operation:
1. Write data to Redis (L2) with version hash
2. Write version hash to local L1
3. Local L1 now has: {key: "hash_of_data"}

Read operation:
1. Check local L1 for version hash of key
2. Check Redis L2 for current version hash
...
**Exam focus:** `RemoteSynchronizedCache` does NOT eliminate Redis reads entirely — it reduces them by serving from local memory when the version hash matches. The version check itself is a lightweight Redis operation.

### 8.4 When to Recommend RemoteSynchronizedCache

- Multi-node PHP deployments (horizontal scaling)
- High read volume where Redis latency is a bottleneck
- Deployments where `/dev/shm` (RAM disk) is available for L1 storage

**When NOT to use it:**
- Single-node deployments (unnecessary complexity)
- Environments where L1 storage is disk-based (negates the benefit)
...
1. Returning `['cat_p']` (generic) instead of `['cat_p', 'cat_p_42']` (specific)
...
- [ ] `mview_state` table tracks last processed `version_id` per indexer
...
- [ ] `reindexList()` is called by MView cron; `reindexAll()` by CLI full reindex

### indexer.xml
- [ ] `id` = CLI identifier, must be unique
- [ ] `view_id` links to mview.xml
- [ ] `class` must implement `IndexerInterface`
- [ ] `dependencies` = other indexers that must run first
- [ ] `shared_index` = shares index table with another indexer
...
- [ ] Blocks implement `IdentityInterface` → `getIdentities()` returns cache tags
...
- [ ] Too-broad tags → all pages invalidate on any product change → cache thrash
- [ ] One `cacheable="false"` block → entire page uncacheable
- [ ] Standard tags: `cat_p`, `cat_p_{id}`, `cat_c`, `cat_c_{id}`, `cms_p`, `cms_p_{id}`

### Redis L2 (RemoteSynchronizedCache)
- [ ] L1 = local process memory (fast), L2 = Redis (shared across nodes)
- [ ] Synchronization via version hash comparison, not full data comparison
- [ ] L1 hash == L2 hash → serve from L1 (no Redis read)
