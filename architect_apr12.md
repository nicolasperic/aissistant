...
```
app/etc/di.xml                    (global bootstrap — highest priority at global scope)
<module>/etc/di.xml               (module global scope)
<module>/etc/<area>/di.xml        (area-specific: frontend, adminhtml, webapi_rest, etc.)
```
...
**What this does:**
- Defines a new injectable "type" named `Grid` (fully-qualified string)
- It is an instance of `Product\Collection` but with `fetchStrategy` overridden
- Any class that type-hints `Grid` in its constructor gets this specific configuration
...
---------------------                   ----------------
Must subclass Product\Collection        Reuse same class with different args
...
```php
// generated/code/Magento/Catalog/Model/Product/Proxy.php
...
{
    private ?HeavyAnalyticsService $subject = null;

    public function __construct(
        private readonly \Magento\Framework\ObjectManagerInterface $objectManager,
        private readonly string $instanceName = HeavyAnalyticsService::class,
...
class MyPlugin
{
    ...
    public function beforeSave(
        ...
        // Must return array of modified arguments, or null to leave unchanged
    ...
    implements \Magento\Framework\Interception\InterceptorInterface
{
    ...
    public function __construct(
        ...
        $this->___init(); // Initialize the interceptor chain
    ...
    // Every public, non-final method gets this treatment
...
trait Interceptor
{
    ...
    public function ___callPlugins(string $method, array $arguments, array $pluginInfo): mixed
    {
...
```bash
grep -A 10 "virtualType" vendor/magento/module-catalog/etc/di.xml
```
...
<virtualType name="catalogProductSearchResultsFactory"
             type="Magento\Framework\Api\SearchResults">
    <arguments>
        <argument name="documentInterface" xsi:type="string">
...
grep -r "catalogProductSearchResultsFactory" vendor/magento/module-catalog/etc/di.xml
```
...
// 3. Every pluggable method is overridden
public function save(...)
{
    $pluginInfo = $this->pluginList->getNext($this->subjectType, 'save');
    // ...
...
// 4. Non-pluggable methods (final, private) are NOT overridden
// 5. The constructor calls ___init() before parent::__construct()
```
...
- Calls parent methods          - Calls subject (lazily loaded) methods
- Used for plugins              - Used for lazy loading
- Generated when plugins exist  - Generated when \Proxy suffix requested
- Uses Interceptor trait        - Uses ObjectManager directly
...
- [ ] Proxy calls `ObjectManager::get()` on first method invocation
...
- [ ] `before` execution order: lowest sortOrder first; `after` order: reverse
