...
Customer clicks "Place Order"
        |
        v
[Synchronous] PlaceOrderInterface::execute()
        |
        v
Order created in DB
        |
...
Consumer processes queue message
        |
        v
PlaceOrderInterface::execute() runs
        |
        v
...
```bash
# Enable AsyncOrder via CLI
bin/magento config:set sales/async_order/enabled 1
...
```xml
<!-- Queue configuration in communication.xml -->
<topic name="async.magento.sales.api.order.place" request="Magento\Sales\Api\Data\OrderInterface" />
```
...
|---|---|---|
| `new` | Order placed, not yet processed | `PlaceOrderInterface::execute()` |
...
{
    public function execute(int $cartId, PaymentInterface $paymentMethod = null): int;
}

// Invoice
use Magento\Sales\Api\InvoiceManagementInterface;
$invoice = $invoiceManagement->prepareInvoice($order);
$invoice->register(); // commits payment capture
...
- `PlaceOrderInterface` is the **single entry point** — customizations to order placement should use this interface's plugins/observers, not override the quote-to-order conversion directly
- Invoice creation triggers **payment capture** for authorize-and-capture flows; for authorize-only, capture happens at invoice time
...
- [ ] Allowed currencies require rate updates via cron; stale rates are a business risk
...
### Async Order (EE)
- [ ] AsyncOrder is **EE only**
- [ ] Publishes order to queue (RabbitMQ recommended); returns immediately to customer
...
- [ ] `PlaceOrderInterface` = single entry point for order placement
- [ ] Invoice creation = triggers payment capture (for authorize-and-capture)
...
- [ ] When customizing order placement: plugin on `PlaceOrderInterface`, not class override
...
- [ ] Plugin > Observer > Preference (class override) for extensibility hierarchy
- [ ] RMA proposal for CE = requires custom module or third-party extension
