...
**Exam focus:** Know these four tables and their roles.

#### `inventory_source`

```sql
-- Physical warehouse/location definition
DESCRIBE inventory_source;
/*
+------------------+--------------+------+-----+---------+
| Field            | Type         | Null | Key | Default |
+------------------+--------------+------+-----+---------+
| source_code      | varchar(255) | NO   | PRI |         |
| name             | varchar(255) | NO   |     |         |
| enabled          | smallint(1)  | NO   |     | 1       |
| description      | text         | YES  |     | NULL    |
| latitude         | decimal(8,6) | YES  |     | NULL    |
| longitude        | decimal(11,6)| YES  |     | NULL    |
...
-- Example data:
SELECT source_code, name, enabled
...
```sql
-- Per-source stock quantities (THE GROSS QTY — NOT salable qty)
DESCRIBE inventory_source_item;
/*
+-------------+--------------+------+-----+---------+
| Field       | Type         | Null | Key | Default |
+-------------+--------------+------+-----+---------+
| source_item_id | int(10)   | NO   | PRI | NULL    |
| source_code  | varchar(255) | NO   | MUL |         |
| sku          | varchar(64)  | NO   | MUL |         |
| quantity     | decimal(12,4)| YES  |     | 0.0000  |
| status       | smallint(1)  | NO   |     | 1       |
+-------------+--------------+------+-----+---------+
*/
...
```sql
-- APPEND-ONLY reservation log
-- NEVER deleted, only compensated with negative entries
DESCRIBE inventory_reservation;
/*
+-------------------+---------------+------+-----+---------+
| Field             | Type          | Null | Key | Default |
+-------------------+---------------+------+-----+---------+
| reservation_id    | int(10)       | NO   | PRI | NULL    |  -- AUTO_INCREMENT
| stock_id          | int(10)       | NO   | MUL |         |
| sku               | varchar(64)   | NO   | MUL |         |
| quantity          | decimal(10,4) | NO   |     | 0.0000  |  -- NEGATIVE = reservation
| metadata          | varchar(255)  | YES  |     | NULL    |  -- JSON: order info
+-------------------+---------------+------+-----+---------+
*/
```
...
#### Priority SSA (`SourceSelectionAlgorithmInterface`)

```
Algorithm: Select sources in priority order until quantity is fulfilled.

Config: Admin > Stores > Inventory > Source Priority (per stock)

Priority Order Example:
  1. warehouse-east (priority 1)
...
use Magento\InventorySourceSelectionApi\Api\SourceSelectionAlgorithmInterface;
use Magento\InventorySourceSelectionApi\Api\Data\SourceSelectionResultInterfaceFactory;
use Magento\InventorySourceSelectionApi\Api\Data\SourceSelectionItemInterfaceFactory;

class MaxStockAlgorithm implements SourceSelectionAlgorithmInterface
{
...
<config xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    ...
    <!-- Register algorithm in the pool -->
    <type name="Magento\InventorySourceSelectionApi\Model\Algorithms\Result\GetDefaultSortedSourcesResult">
        <arguments>
            <argument name="algorithms" xsi:type="array">
                <item name="max_stock" xsi:type="object">
...
**Exam focus:**
- Custom SSAs implement `SourceSelectionAlgorithmInterface` and are registered via `di.xml`.
- The service entry point is `SourceSelectionServiceInterface::execute(request, algorithmCode)`.
- SSA code is a string identifier — it's how you switch algorithms per business logic.
...
Q: "Order fulfillment from wrong warehouse?"
   ...
   |-- Check: source priorities in Admin > Stores > Inventory
   |-- Check: are sources enabled and linked to the correct stock?

Q: "Custom fulfillment logic needed?"
   |-- Implement SourceSelectionAlgorithmInterface
   |-- Register in di.xml as algorithm pool item
...
```bash
# Place order via Magento admin or storefront for WS12-XS-Orange x 2
# Then inspect:
```
...
// In a controller, observer, or test script:
...
- [ ] **MSI core tables**: `inventory_source`, `inventory_source_item`, `inventory_reservation`, `inventory_stock`
- [ ] `inventory_source` = physical warehouse/location definitions (with lat/long for Distance SSA)
- [ ] `inventory_source_item` = gross physical quantity per source per SKU
...
- [ ] **Salable qty = SUM(source_item.qty) + SUM(reservation.qty)** — reservation sum is typically negative
- [ ] `inventory_source_item.quantity` alone does NOT reflect available quantity — reservations must be included
...
- [ ] **Priority SSA** = default algorithm; selects sources by configured priority order; no external APIs
- [ ] **Distance SSA** = selects nearest source to delivery address; **requires Google Maps API** configuration
- [ ] Custom SSA: implement `SourceSelectionAlgorithmInterface`; register in `di.xml` algorithm pool
- [ ] Custom SSA invoked via `SourceSelectionServiceInterface::execute($request, $algorithmCode)`
...
- [ ] Use `GetProductSalableQtyInterface::execute($sku, $stockId)` for accurate available quantity in new code
- [ ] `StockRegistry::getStockItemBySku()` reads legacy table — **avoid for accurate MSI quantities**
...
- [ ] Custom fulfillment routing → implement custom SSA, not Observer pattern on order placement
