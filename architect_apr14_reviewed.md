...
**Important:** Real core interface methods do NOT use PHP return type hints. They rely entirely on `@return` docblock annotations. The REST API layer uses PHPDoc reflection to determine serialization types — it reads `@return` and `@param` annotations, not PHP type declarations. This is why the docblock is critical for `@api` interfaces.
...
| `$product->getPrice()` with `@return float|null` | Documentable, REST/GraphQL schema auto-generated via docblock reflection, mockable |

- **Exam focus:** The REST API **auto-generates** endpoints from `@api`-annotated interfaces. The `@return` docblock annotation on getters determines the JSON serialization type — the framework reads docblocks via reflection, not PHP type hints (which do not exist on most core `@api` interfaces). If the `@return` annotation is missing or says `mixed`, the REST layer cannot reliably serialize it.
...
The concrete `\Magento\Catalog\Model\Product` implements `ProductInterface` by extending `\Magento\Catalog\Model\AbstractModel`, which itself extends `\Magento\Framework\Model\AbstractExtensibleModel`. The inheritance chain is:
...
`AbstractExtensibleModel` provides:
...
- **Exam focus:** `addFilter()` on `SearchCriteriaBuilder` creates a **new FilterGroup** each time it is called — confirmed by reading the source: `addFilter()` internally calls `addFilters([...])` with a single-item array, which appends a new FilterGroup to the array. `addFilters([filter1, filter2])` puts both into the **same FilterGroup** (OR). Confusing them produces wrong query logic with no error — a silent bug.
...
**Important:** Like all `@api` interfaces in the framework, `SearchResultsInterface` methods do not carry PHP return type hints — they use docblock `@return` annotations only. The REST serializer reads these annotations.
...
- **Exam focus:** `getItems()` returns `array` (of typed objects), not the object itself being iterable, because REST API serializers use reflection on the **docblock `@return` annotation** to determine the JSON structure.
...
**Note:** Methods in the real interface have **no PHP parameter or return type hints** — type information is conveyed entirely through docblocks. This is consistent across all framework-era `@api` interfaces.
...
- **Exam focus:** The exception class hierarchy matters: `NoSuchEntityException extends LocalizedException extends \Exception`. The REST API maps specific exception types to HTTP codes via `Magento\Framework\Webapi\ErrorProcessor::maskException()`.

### Exception-to-HTTP Mapping

The mapping is implemented in `vendor/magento/framework/Webapi/ErrorProcessor.php`. The logic is:

1. If `instanceof NoSuchEntityException` → **404 Not Found**
2. If `instanceof AuthorizationException` or `AuthenticationException` → **401 Unauthorized**
3. If any other `instanceof LocalizedException` (including `CouldNotSaveException`, `CouldNotDeleteException`, `StateException`, `InputException`) → **400 Bad Request**
4. Non-`LocalizedException` exceptions (e.g., bare `\Exception`, `\RuntimeException`) → **500 Internal Server Error**

| Exception | HTTP Status | Notes |
|---|---|---|
| `NoSuchEntityException` | 404 Not Found | Explicit case in ErrorProcessor |
| `AuthorizationException` | **401 Unauthorized** | Not 403 — maps to `HTTP_UNAUTHORIZED` |
| `AuthenticationException` | 401 Unauthorized | Same as AuthorizationException |
| `InputException` | 400 Bad Request | Extends `AbstractAggregateException extends LocalizedException` |
| `CouldNotSaveException` | 400 Bad Request | Extends `AbstractAggregateException extends LocalizedException` |
| `CouldNotDeleteException` | 400 Bad Request | Extends `LocalizedException` directly |
| `StateException` | 400 Bad Request | Extends `LocalizedException` directly |
| `LocalizedException` (generic) | 400 Bad Request | The `else` fallthrough for all LocalizedExceptions |
| Non-LocalizedException | 500 Internal Server Error | Bare `\Exception`, `\RuntimeException`, etc. |

**Exam trap:** `AuthorizationException` maps to **401 Unauthorized**, NOT 403 Forbidden. `CouldNotSaveException`, `CouldNotDeleteException`, `StateException`, and generic `LocalizedException` all map to **400 Bad Request**, NOT 500 — because they all extend `LocalizedException`.
...
interface ProductExtensionInterface extends \Magento\Framework\Api\ExtensionAttributesInterface
{
     ...
     * @return int|null
     */
    public function getVendorWarrantyYears();

    /**
     * @param int $vendorWarrantyYears
     * @return $this
     */
    public function setVendorWarrantyYears($vendorWarrantyYears);
     ...
     * @return $this
     */
    public function setVendorCertifications(array $vendorCertifications);
}
```
...
The `JoinProcessorInterface` lives at `Magento\Framework\Api\ExtensionAttribute\JoinProcessorInterface`.
...
class ProductRepository implements \Magento\Catalog\Api\ProductRepositoryInterface
{
    ...
    // Note: declared as protected $instancesById = [] (no array type hint in actual code)
...
### Data Interfaces
- [ ] Typed getters/setters exist because REST API uses **`@return` docblock annotations** for JSON serialization (not PHP return type hints — core `@api` interfaces have no PHP type hints)
...
- [ ] `delete()` throws `StateException` (not `CouldNotDeleteException`) in `ProductRepositoryInterface`

### Exception-to-HTTP Mapping (ErrorProcessor)
- [ ] `NoSuchEntityException` → **404** Not Found
- [ ] `AuthorizationException` / `AuthenticationException` → **401** Unauthorized (NOT 403)
- [ ] All other `LocalizedException` subclasses (`CouldNotSaveException`, `CouldNotDeleteException`, `StateException`, `InputException`) → **400** Bad Request
- [ ] Non-`LocalizedException` → **500** Internal Server Error
...
- [ ] `JoinProcessorInterface` namespace: `Magento\Framework\Api\ExtensionAttribute\JoinProcessorInterface`
