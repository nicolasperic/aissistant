...
- `Magento_Backend::all` (title="Allow everything") is the direct child of `Magento_Backend::admin` — granting it gives full admin access (confirmed in `module-backend/etc/acl.xml:11`)
...
All Escaper methods confirmed in `vendor/magento/framework/Escaper.php`: `escapeHtml()`, `escapeHtmlAttr()`, `escapeUrl()`, `escapeJs()`, `escapeCss()`, `escapeJsQuote()`, `escapeXssInUrl()`, `escapeQuote()`.
...
> **Note:** The root element is `<csp_whitelist>` (not `<csp_whitelist.xml>`), and the schema URI uses a colon before `etc`: `urn:magento:module:Magento_Csp:etc/csp_whitelist.xsd` — confirmed in `module-paypal/etc/csp_whitelist.xml` and `module-csp/etc/csp_whitelist.xsd`.
...
**Supported providers (out of box) — confirmed in `module-two-factor-auth/Model/Provider/Engine/`:**

| Provider | Engine Class |
|---|---|
| Google Authenticator | `Google.php` |
| Authy | `Authy.php` |
| Duo Security | `DuoSecurity.php` |
| U2F / YubiKey | `U2fKey.php` |
...
# In development environments only — disable specific providers via config
# Config path: twofactorauth/general/force_providers (Admin > Config > Security > 2FA)
bin/magento config:set twofactorauth/general/force_providers ""
```

**Exam focus:** In an architect exam scenario where "we need to disable 2FA for automation testing," the correct architectural answer is to use the `--no-interaction` flag or configure a programmatic 2FA bypass for CI only, not to disable it in production. On Adobe Commerce Cloud, the `Magento_AdminAdobeImsTwoFactorAuth` module (in `module-admin-adobe-ims`) can replace standard 2FA with Adobe IMS SSO.
...
Admin Session Lifetime (seconds): 900 (15 min default — set in module-security/etc/config.xml)
```

**Config path:** `admin/security/session_lifetime` (default 900 seconds, defined in `module-security/etc/config.xml`).
...
**Configuration paths (confirmed in `module-user/etc/config.xml`):**

| Setting | Config Path | Default |
|---|---|---|
| Max login failures | `admin/security/lockout_failures` | 6 |
...
2. After `lockout_failures` failures, `lock_expires` is set
...
**Vault token storage in database (confirmed schema from `module-vault/etc/db_schema.xml`):**

```sql
-- vault_payment_token table — key columns
entity_id | customer_id | website_id | public_hash | payment_method_code | type | created_at | expires_at | gateway_token | details | is_active | is_visible
```

The `details` column is TEXT type and stores a JSON blob.
...
**Step 4: Read the ACL tree context (confirmed in `module-catalog/etc/acl.xml`)**
...
acl.xml: Magento_Catalog::products is child of Magento_Catalog::catalog_inventory
          which is child of Magento_Catalog::catalog
          |
          v
...
- [ ] `Magento_Backend::all` (direct child of admin) grants full admin access — confirmed in `module-backend/etc/acl.xml`
...
- [ ] `escapeCss()` — use inside CSS context
...
- [ ] `csp_whitelist.xml` root element is `<csp_whitelist>` (not `<csp_whitelist.xml>`)
- [ ] Schema URI: `urn:magento:module:Magento_Csp:etc/csp_whitelist.xsd` (colon before `etc`, not slash)
- [ ] `csp_whitelist.xml` lives in `Vendor/Module/etc/` — adds CSP exceptions per module
...
- [ ] 2FA providers (confirmed engines): Google.php, Authy.php, DuoSecurity.php, U2fKey.php
- [ ] Adobe Commerce Cloud option: `Magento_AdminAdobeImsTwoFactorAuth` for IMS SSO
- [ ] Admin URL customization via `backend/frontName` in `env.php` or `setup:config:set --backend-frontname`
- [ ] Max login failures config path: `admin/security/lockout_failures` (default: 6)
- [ ] Lockout period config path: `admin/security/lockout_threshold` (default: 30 minutes)
- [ ] Session lifetime: `admin/security/session_lifetime` (default: 900 seconds = 15 min, in module-security/etc/config.xml)
...
- [ ] `failures_num` and `lock_expires` columns in `admin_user` table track lockout state
...
- [ ] `vault_payment_token` columns: entity_id, customer_id, website_id, public_hash, payment_method_code, type, created_at, expires_at, gateway_token, details, is_active, is_visible
...
- [ ] `vault_payment_token.details` column (TEXT) stores JSON with masked card info only
