...
> **Note:** `Magento\Framework\Cache\Backend\Redis` is the Magento wrapper class (confirmed in `setup/src/Magento/Setup/Model/ConfigOptionsList/Cache.php`). Underneath it wraps `Cm_Cache_Backend_Redis` from `colinmollenhour/cache-backend-redis` — but the correct string to use in `env.php` is the Magento class name.
...
- **Exam focus:** `setup:upgrade` internally runs `setup:db-schema:upgrade` and `setup:db-data:upgrade` (note: hyphen-separated, not colon-separated). It also applies `UpgradeSchema`, `UpgradeData`, `InstallSchema`, `InstallData` scripts (for pre-2.3 declarative schema modules). Confirmed command names: `DbSchemaUpgradeCommand::NAME = 'setup:db-schema:upgrade'` and `DbDataUpgradeCommand::NAME = 'setup:db-data:upgrade'`.
...
Browser -> Nginx/Apache -> pub/index.php
                    ...
                    App\Http (Front Controller)
                                |
                                v
                    Router::match() [4 routers, in sortOrder]
                    +----------------------------------------------+
                    | 1. URL Rewrite Router  (sortOrder 20)        |
                    | 2. Base Router/standard (sortOrder 30)       |
                    | 3. CMS Router           (sortOrder 60)       |
                    | 4. Default Router (404) (sortOrder 100)      |
                    +----------------------------------------------+
                                |
                                v
                    Controller::execute()
...
> **Correction:** The router execution order is determined by `sortOrder` in `di.xml`, not by name alphabetically. URL Rewrite Router fires **first** (sortOrder 20), followed by Base/Standard Router (30), CMS Router (60), and Default Router (100). Sources: `module-url-rewrite/etc/frontend/di.xml`, `module-store/etc/frontend/di.xml`, `module-cms/etc/frontend/di.xml`.
...
- [ ] Router order by sortOrder: URL Rewrite (20) → Base/Standard (30) → CMS (60) → Default (100)
- [ ] `setup:upgrade` calls `setup:db-schema:upgrade` and `setup:db-data:upgrade` (hyphens, not colons)
