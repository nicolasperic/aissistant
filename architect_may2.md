...
; JIT mode digits: OTRF
; O = optimization level (0-5)
; T = trigger (0=off, 1=profiling run, 2=hot function, 3=trace, 4=on first exec, 5=re-JIT)
; R = register allocation (0=none, 1=local, 2=global)
; F = JIT function call (0=no, 1=yes)
; Common values:
opcache.jit=on       ; equivalent to 1254 (tracing JIT, recommended)
...
"^2.0"      # >= 2.0.0 < 3.0.0  (caret: minor/patch updates)
"~2.1"      # >= 2.1.0 < 3.0.0  (tilde: patch updates only for 2.1.x)
"~2.1.0"    # >= 2.1.0 < 2.2.0  (tilde with patch: patch updates only)
...
- [ ] `^2.0` = `>=2.0.0 <3.0.0`; `~2.1` = `>=2.1.0 <3.0.0`; `~2.1.0` = `>=2.1.0 <2.2.0`
