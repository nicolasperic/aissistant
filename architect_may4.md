...
> **Exam focus:** For **inventory operations in Adobe Commerce (EE)**, RabbitMQ is **required** (not optional) because the MSI module publishes reservation messages that must be processed in order with guaranteed delivery.
...
### Scenario 4: Module Modifying a Final Class

> "You need to add logging to `Magento\Payment\Gateway\Command\GatewayCommand::execute()`. The class is marked final. What is the correct approach?"

**Analysis:**
- Plugin is **impossible** — class is final, interceptor generation will fail
...
- Decorator pattern: inject your logging class that wraps `GatewayCommand`

**Correct Answer:**
1. Check for `payment_gateway_command_execute_before/after` events
2. If none exist, implement a **Decorator** wrapping `GatewayCommandInterface`
...
- [ ] **RabbitMQ** = high-volume; guaranteed delivery; built-in DLQ; required for EE MSI
