...
```php
// Magento\Quote\Api\CartItemProcessorInterface
...
use Magento\Quote\Api\CartItemProcessorInterface;
...
<type name="Magento\Quote\Model\Cart\CartItemOptionsProcessor">
    <arguments>
...
> **Exam focus:**
> - `CartItemProcessorInterface` has **two methods**: `convertToBuyRequest()` (inbound) and `processOptions()` (outbound). Know which direction each flows.
> - Custom data must be stored on the quote item as a **quote item option** (not a flat column) to survive serialization.
> - The processor is registered in `CartItemOptionsProcessor`'s `cartItemProcessors` array — not via an event.
...
### TotalsCollectorInterface

```php
// Magento\Quote\Api\Data\TotalsCollectorInterface (internal SPI)
// The real extension point is registering via sales.xml / di.xml:
...
abstract class AbstractTotal
{
     ...
     * MUST call parent or replicate base behaviour if overriding.
     *
     ...
     * Return null to exclude from frontend display.
     *
     * @param \Magento\Quote\Model\Quote         $quote
     * @param \Magento\Quote\Model\Quote\Address\Total $total
     * @return array|null
...
<config xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    ...
    <section name="quote">
        <group name="totals">
            ...
            <!-- Place AFTER tax (450) and shipping (300) if you depend on them. -->
...
| Total Code | Default Sort Order | Depends On |
|---|---|---|
| `subtotal` | 100 | (base) |
| `shipping` | 300 | subtotal |
| `tax` | 450 | shipping |
| `discount` | 200 | subtotal |
| `grand_total` | 550 | all |
| Your custom total | 400–549 (typical) | varies |
...
> - Always use `$total->setTotalAmount()` / `$total->setBaseTotalAmount()` rather than setting address fields directly. The `$code` you pass must match `$this->getCode()`.
> - `fetch()` feeds the **JavaScript totals renderer** — return `null` to suppress display without affecting calculation.
...
InvoiceOrderInterface::execute($orderId, $capture, $items, $notify, ...)
        ...
        |
        +-> Emitter: sales_order_invoice_pay        // event
...
> **Exam focus:**
> - `InvoiceOrderInterface::execute()` with `$capture = true` triggers **online payment capture** through the payment method's `capture()` command.
...
<!-- 4. Register the Facade as the payment method model -->
<type name="VendorName\ModuleName\Model\Payment\EePaymentMethod">
    <arguments>
        <argument name="commandManager" xsi:type="object">
            Magento\Payment\Gateway\Command\CommandManager
        </argument>
    </arguments>
</type>
```
...
> - `vault_authorize` command is required for the **Vault** (stored card) flow — its absence causes vault payments to fail silently.
...
> - Vault tokens are stored in `vault_payment_token` table, linked to both customer and order.
> - `VaultPaymentInterface` extends `MethodInterface` — a vault method is a **separate payment method code** (e.g., `braintree_cc_vault`) that delegates to the parent gateway via token.
...
- [ ] Registered in `CartItemOptionsProcessor`'s `cartItemProcessors` DI array
...
- [ ] Always call `parent::collect()` at the top of your `collect()` method
...
- [ ] `fetch()` returns array for JS totals display; return `null` to hide from frontend
- [ ] Registered in `sales.xml` under `section="quote" > group="totals"`
- [ ] Sort order: tax=450, shipping=300, grand_total=550 — place custom totals accordingly
