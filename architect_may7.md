...
2. [The Top 10 Tricky Concepts](#2-the-top-10-tricky-concepts)
   - [2.1 Plugin sortOrder + Alphabetical Module Tiebreaker](#21-plugin-sortorder--alphabetical-module-tiebreaker)
...
### 2.1 Plugin sortOrder + Alphabetical Module Tiebreaker

#### The Core Rule

Plugins on the same method are sorted by `sortOrder`. When two plugins share **the same `sortOrder` value**, Magento breaks the tie **alphabetically by module name** — not by class name, not by XML file path.
...
Intercepted Method Call
        |
        v
[before plugins]  --> sorted by sortOrder ASC (then alpha by module)
        |
        v
...
Execution:  Vendor_Alpha runs BEFORE Vendor_Beta
```
...
**Answer:** `Vendor_Apple` — because `A` comes before `Z`. The module name (the folder/namespace component registered in `registration.php`) is the tiebreaker, **not** the plugin `name` attribute.

- **Exam focus:** The tiebreaker is **module name** alphabetically, not plugin name, not class name.
...
The `inventory_reservation` table grows over time. Adobe provides a CLI tool:

```bash
# Compensate completed orders (add positive reservations for shipped/cancelled)
...
- **Exam focus:** Salable quantity = `source_item.qty` + `SUM(reservations)`. You must sum the reservation log.
...
stage:
  ...
  build:
    SCD_STRATEGY: compact  # Strategies: standard, quick, compact
    SCD_THREADS: 4
```
...
- **Exam focus:** `SCD_STRATEGY` options: `standard` (all locales, all themes), `quick` (faster, fewer symlinks), `compact` (minimizes disk space).
...
|-----------|------------------|-----|
| Plugins | **Yes** | `sortOrder` attribute, then alphabetical tiebreaker |
...
- [ ] Same sortOrder tiebreaker = **module name alphabetically**
...
- [ ] Plugin order IS guaranteed; observer order is NOT
...
- [ ] Use `inventory:reservation:create-compensations` to clean up

### Static Content Deploy
- [ ] `SCD_ON_DEMAND=true` in production = anti-pattern (latency spikes)
- [ ] SCD strategies: `standard`, `quick`, `compact`
