...
// ViewModels implement this interface:
namespace Magento\Framework\View\Element;

interface ViewModelInterface
{
    // Marker interface — no required methods
    // Convention: add public methods that templates call
}
```
...
**Exam focus:** ViewModels must implement `Magento\Framework\View\Element\Block\ArgumentInterface` (which extends `ViewModelInterface`). This is the correct interface — not `ViewModelInterface` directly — in current Magento versions.
...
**Location:** `Magento\Ui\view\base\ui_component\etc\definition.xml`
...
<components xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    ...
    <listing class="Magento\Ui\Component\Listing">
...
```xml
<!-- view/adminhtml/ui_component/product_listing.xml (simplified) -->
<listing xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
         xsi:noNamespaceSchemaLocation="urn:magento:module:Magento_Ui:etc/ui_configuration.xsd">

    <!-- DataSource: wires to the DataProvider PHP class -->
    <argument name="data" xsi:type="array">
        <item name="js_config" xsi:type="array">
    ...
    <!-- The DataSource component definition -->
...
{
    // Returns field metadata (column definitions, form field config)
    public function getMeta(): array;

    // Returns the actual data (rows for grids, field values for forms)
    public function getData(): array;

    // Returns the component name
    public function getName(): string;

    // Returns the primary field (usually 'entity_id' or 'id')
    public function getPrimaryFieldName(): string;

    // Returns the request parameter name used to identify the record
    ...
    // Returns search result fields (for SearchCriteria pattern)
...
Look for the `<dataSource>` element:
...
<dataSource name="product_listing_data_source">
    <argument name="dataProvider" xsi:type="configurableObject">
        ...
        <argument name="addFilterStrategies" xsi:type="array">
            <item name="websites" xsi:type="object">
                Magento\Catalog\Ui\DataProvider\Product\AddWebsitesFilterToCollection
            </item>
        </argument>
    </argument>
</dataSource>
...
Search for `product_listing_data_source`:

```xml
<type name="Magento\Framework\View\Element\UiComponent\DataProvider\CollectionFactory">
    <arguments>
        <argument name="collections" xsi:type="array">
            <item name="product_listing_data_source" xsi:type="string">
...
ProductDataProvider (extends DataProvider)
    |
    | injects CollectionFactory
    v
CollectionFactory->create('product_listing_data_source')
    |
    | looks up in di.xml collections array
    | key = 'product_listing_data_source'
    v
Magento\Catalog\Ui\DataProvider\Product\ProductCollection
    |
...
```php
// These strategies allow extending the grid without modifying the DataProvider
// Injected via di.xml as arrays — open/closed principle in action
...
Add a new column to the product admin listing:
    ...
    +-- Is it data from a JOIN or separate table?
            |
            +-- YES --> Create AddFieldToCollectionInterface strategy
...
- [ ] `definition.xml` — maps component type names (e.g., `listing`, `form`) to PHP classes
- [ ] Located in `view/*/ui_component/` directory
- [ ] `DataProviderInterface` — supplies metadata and data to the component
- [ ] `AbstractDataProvider` — base for form DataProviders
- [ ] `DataProvider` (UiComponent\DataProvider) — base for listing DataProviders, implements SearchCriteria
- [ ] DataProvider name must match key in `CollectionFactory` di.xml argument exactly
- [ ] `addFieldStrategies` / `addFilterStrategies` — extension points for grid customization
...
- [ ] `xsi:type="configurableObject"` — for class references in ui_component dataSource
