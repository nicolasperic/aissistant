...
AUTHORIZE ONLY (card present, delayed capture)
  ...
  Customer places order
      |
      v
  PaymentInterface::authorize() called
  - Holds funds on card
  ...
  Admin reviews / ships
      |
      v
  PaymentInterface::capture() called
...
AUTH + CAPTURE (immediate)
  ...
  Customer places order
      |
      v
  PaymentInterface::authorize() + capture() in same call
  OR PaymentInterface::sale() — depends on gateway
```
...
Magento Checkout
  ...
  |
  v
Braintree Payment Method (implements PaymentInterface)
  |
...
```php
// Implementing PaymentInterface correctly
...
| Interface | Purpose |
|---|---|
| `PaymentInterface` | Core payment method contract |
