...
| `setup_version` | (Deprecated in favour of `db_schema.xml` declarative schema but still seen in exam scenarios) |
...
| `webapi_rest` | `pub/rest/` | REST API | `etc/webapi_rest/` |
| `webapi_soap` | `pub/soap/` | SOAP API | `etc/webapi_soap/` |
| `graphql` | `pub/graphql/` | GraphQL API | `etc/graphql/` |
...
Area: crontab
  ...
  -> Jobs defined in crontab/di.xml or crontab.xml
```
...
+-- cron_groups.xml             (cron group definitions)
...
> - `sortOrder` takes precedence over sequence. Sequence is the tiebreaker.
...
Magento detects this during setup:di:compile and throws:
  Magento\Framework\Exception\LocalizedException:
  "Circular dependency: Module Vendor_A depends on Vendor_B,
   which depends on Vendor_A"
```

> **Exam focus:**
> - Circular sequence dependencies cause a **compile-time error**, not a runtime error.
...
- [ ] `module.xml` declares module name and schema version, plus optional `<sequence>`
...
- [ ] `sortOrder` takes precedence over sequence as tiebreaker
- [ ] Circular sequence dependencies cause `setup:di:compile` error, not runtime error
- [ ] Sequence is transitive (A after B after C = A loads after C)

### Area Codes

- [ ] Six areas: `frontend`, `adminhtml`, `webapi_rest`, `webapi_soap`, `graphql`, `crontab`
