...
9. [Zero-Downtime Deployment — Blue/Green on Pro](#9-zero-downtime-deployment--bluegreen-on-pro)
...
**Exam focus:**
- Schema patches implement `\Magento\Framework\Setup\Patch\DataPatchInterface` or `SchemaPatchInterface`
- Patches are **idempotent by design** — the `getAliases()` and `getDependencies()` methods ensure correct ordering
...
|----------|-------------|-----------|------------|----------|
| `compact` | Symlinks shared files, copies unique | Medium | Low | Cloud Pro (default) |
| `quick` | Generates minimal set, symlinks rest | Fast | Low | Large catalogs, many locales |
| `standard` | Full copy of all files for each locale | Slow | High | On-premise, debugging |

### Strategy Deep Dive

```bash
# compact strategy (default for Cloud Pro)
...
- `quick` is faster because it **defers some file resolution** to runtime — not suitable for all setups
- `standard` is the **safest but slowest** — never use in Cloud production
- The strategy is set in `.magento.env.yaml` under `SKIP_SCD` or `SCD_STRATEGY` variables
...
## 9. Zero-Downtime Deployment — Blue/Green on Pro
...
### How Blue/Green Works on Cloud Pro

```
1. New code built into slug (artifact)
2. Slug deployed to GREEN environment
3. GREEN runs setup:upgrade, cache flush
4. Load balancer atomically switches: BLUE --> GREEN
5. Users now hit GREEN (new code)
6. BLUE kept warm for rollback
```

**Exam focus:**
- Blue/Green is available on **Cloud Pro** — **not** on Cloud Starter
- The "switch" is nearly instantaneous — this is what enables "zero-downtime" on Pro
- Zero-downtime is not truly zero — there is a brief DNS/LB switch period
- **Database migrations** (`setup:upgrade`) still run on GREEN before the switch — if they modify shared tables, this requires backward-compatible migrations
- The shared database is a constraint: BLUE and GREEN share the same DB during the switch — schema changes must be backward compatible

### Backward-Compatible Migration Pattern

```php
// WRONG for Blue/Green — removes column while BLUE still reads it
class RemoveOldColumn implements SchemaPatchInterface
{
    public function apply(): void
    {
        ...
        // BLUE is still running and reading old_column = ERRORS
    }
}

// CORRECT — three-deploy pattern for Blue/Green
// Deploy 1: Add new column, keep old column
// Deploy 2: Migrate data, switch code to use new column
// Deploy 3: Remove old column (BLUE now on new code too)
```
...
- [ ] `compact` = symlinks shared, copies unique — default Cloud Pro — **watch for broken symlinks**
- [ ] `quick` = minimal generation, runtime fallback — fastest build
...
- [ ] Blue/Green available on **Cloud Pro only** — not Starter
- [ ] New code deployed to GREEN, LB switches BLUE → GREEN
- [ ] DB migrations must be **backward compatible** during switch
- [ ] Three-deploy pattern for breaking schema changes
- [ ] "Zero downtime" is near-zero — LB switch has brief window
