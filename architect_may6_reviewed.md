...
### 2.3 sortOrder + Module Load Sequence Tiebreaker
...
Tiebreaker when sortOrder is equal:
  -> Module load sequence (sequence in app/etc/config.php / composer.json require order)
  -> NOT alphabetical by module name — module load order, not class name order
  -> before/around: earlier-loaded module's plugin runs first
  -> after:         earlier-loaded module's plugin runs last (still reversed)
```

<!-- CORRECTED: Original stated "Alphabetical order by module name (A before B)" — verified WRONG.
     PluginList.php has no alphabetical tiebreaker. Equal sortOrder resolves by module load sequence
     as defined in module registration/load order. Source: PluginList.php verification (Day 24). -->

**Exam focus:** Default `sortOrder` is `10` if not specified. Always set explicit sortOrder in real code to avoid module-load-sequence tiebreaker surprises.
...
stage:
  build:
    SCD_STRATEGY: quick          # quick (default)|standard|compact
    SCD_ON_DEMAND: false
    SKIP_SCD: false
...
<!-- NOTE: SCD_STRATEGY default is `quick` (not `compact`). `compact` uses symlinks — dev/test only.
     `standard` generates full copies per locale. `quick` deploys only active themes (Cloud default). -->
...
**Exam focus:** The reservation table grows indefinitely. Use the CLI commands to diagnose and fix inconsistencies:
- `inventory:reservation:list-inconsistencies` — shows unresolved reservation mismatches
- `inventory:reservation:create-compensations` — creates compensating entries to resolve inconsistencies

<!-- CORRECTED: Original stated `inventory:reservations:cleanup` — this command does NOT exist.
     Actual commands (confirmed in module-inventory-reservation-cli/Command/):
     `inventory:reservation:list-inconsistencies` and `inventory:reservation:create-compensations`.
     Note the singular "reservation" (not plural "reservations") in the command namespace. -->
...
Requirements (ALL must be true):
  ...
  3. Query must be marked @cache(cacheIdentity: "...") in schema
...
type Query {
    products(
        ...
        pageSize: Int = 20
        currentPage: Int = 1
    ): Products @cache(cacheIdentity: "Magento\\CatalogGraphQl\\Model\\Resolver\\Product\\Identity")
...
<!-- CORRECTED: Original had wrong cacheIdentity class: "Magento\\CatalogGraphQl\\Model\\Resolver\\Cache\\ProductsQuery"
     Actual class confirmed in module-catalog-graph-ql/etc/schema.graphqls:12:
     "Magento\\CatalogGraphQl\\Model\\Resolver\\Product\\Identity" -->

### 8.4 X-Magento-Cache-Id Composition

The `X-Magento-Cache-Id` header value is a SHA-256 hash of context factors registered via `CacheIdFactorProviderInterface` in `etc/graphql/di.xml`. Default factors:

| Factor | Provider | Source |
|--------|----------|--------|
| `store` | `StoreProvider` | Current store code |
| `currency` | `CurrencyProvider` | Current currency code |
| `customergroup` | `CustomerGroupProvider` | Customer group ID |
| `customertaxrate` | `CustomerTaxRateProvider` | Customer tax rate |
| `isloggedin` | `IsLoggedInProvider` | Authentication state |

<!-- CORRECTED: Original stated "computed from: store code + currency + customer group (guest) + HTTP Vary headers".
     HTTP Vary headers are NOT factors in the cache ID. The actual factors are the 5 providers above,
     confirmed in module-store-graph-ql/etc/graphql/di.xml and module-customer-graph-ql/etc/graphql/di.xml.
     The hash also includes a random salt stored in env.php at path `cache/graphql/id_salt`. -->
...
### 8.5 GraphQL vs REST Caching Comparison
...
class CustomProductData implements ResolverInterface
{
    public function resolve(
        ...
        // Correct method: getIsCustomer() — NOT isCustomerLoggedIn()
        // $context->getExtensionAttributes()->getIsCustomer()
        // If true, response CANNOT be cached (auth required)
...
<!-- CORRECTED: Original used $context->getExtensionAttributes()->isCustomerLoggedIn() — method does NOT exist.
     Correct method: getIsCustomer() — confirmed in module-customer-graph-ql/Model/Resolver/Customer.php:56
     and IsLoggedInProvider.php:33. Returns bool — true if customer is authenticated. -->
...
[New order created]
        ...
        v
+---------------------+
|  new                |  <- order placed, payment not yet confirmed
+---------------------+
        |
...
Refunded after complete:
+------------------+
|    closed        |  <- fully refunded/credited
+------------------+

At any eligible state:
+------------------+
|   canceled       |  <- only from new/pending_payment/processing
+------------------+

+------------------+
|   holded         |  <- manually held, can return to prior state
+------------------+
```

<!-- CORRECTED: Original diagram showed a "pending" state (box labeled "pending") as a separate state
     transitioning to "pending_payment". There is NO standalone `pending` state in Magento.
     Confirmed in module-sales/Model/Order.php (Day 15 verified facts): constants are
     STATE_NEW='new', STATE_PENDING_PAYMENT='pending_payment', STATE_PROCESSING='processing',
     STATE_COMPLETE='complete', STATE_CLOSED='closed', STATE_CANCELED='canceled',
     STATE_HOLDED='holded', STATE_PAYMENT_REVIEW='payment_review'. No STATE_PENDING. -->

### 10.3 States Reference Table

| State | Code | Description | Can Cancel? |
|-------|------|-------------|-------------|
| New | `new` | Order created, payment pending | Yes |
| Pending Payment | `pending_payment` | Awaiting payment gateway | Yes |
| Processing | `processing` | Payment received, fulfillment | Yes |
| Complete | `complete` | Shipped and invoiced | No (credit memo only) |
| Closed | `closed` | Refunded/credited | No (terminal) |
| Canceled | `canceled` | Order canceled | No (terminal) |
| On Hold | `holded` | Manually held | No (unhold first) |
| Payment Review | `payment_review` | Fraud check in progress | Yes |
...
```php
<?php
// Unit Test — extends \PHPUnit\Framework\TestCase directly
...
// (provides $this->_objectManager; for non-controller tests use Bootstrap::getObjectManager())
...
<!-- CORRECTED: Original had `extends \Magento\TestFramework\TestCase\AbstractIntegration` — this class
     DOES NOT EXIST. Available test case classes in dev/tests/integration/framework/Magento/TestFramework/TestCase/:
     AbstractController.php, AbstractBackendController.php, AbstractConfigFiles.php, AbstractIntegrity.php.
     For integration tests that need $this->_objectManager, extend AbstractController.
     For plain integration tests, extend \PHPUnit\Framework\TestCase and use
...
- [ ] **Module load sequence** tiebreaker when `sortOrder` is equal (NOT alphabetical by module name)
...
- [ ] FPC invalidated by entity save via cache tags (`X-Magento-Tags` response header)
- [ ] Cache tags = scalable invalidation; cleaning full type = brute force
- [ ] `X-Magento-Cache-Id` = SHA-256 hash of: store + currency + customer group + tax rate + is logged in
...
- [ ] Inconsistency CLI: `inventory:reservation:list-inconsistencies` / `inventory:reservation:create-compensations`
...
- [ ] `X-Magento-Cache-Id` computed from store + currency + customer group + tax rate + is logged in (SHA-256 + salt)
- [ ] Check auth in resolver: `$context->getExtensionAttributes()->getIsCustomer()` (NOT `isCustomerLoggedIn()`)
...
### Order State Machine
- [ ] Valid state codes: `new`, `pending_payment`, `processing`, `complete`, `closed`, `canceled`, `holded`, `payment_review`
- [ ] NO standalone `pending` state — that would be `pending_payment`
...
- [ ] Integration base class: `AbstractController` (provides `$this->_objectManager`) or `Bootstrap::getObjectManager()` for plain `TestCase`
