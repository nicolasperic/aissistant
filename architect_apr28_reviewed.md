...
- When `false`: HTML templates are minified during build (slows build)
- When `true`: HTML minification is **skipped entirely** — static templates are generated without minification, stored in `./init/` and copied to live mounts at deploy time without any minification step
- On Cloud, the build container is ephemeral and shared — minimizing build time improves pipeline efficiency; the payload difference from unminified HTML is negligible in most production scenarios
...
```
  ...
  +------------------+  +------------------+
         ...
         +----------+-----------+
                    |
              Snapshot ID
              (stored in Adobe Cloud
               infrastructure — NOT Fastly;
               Fastly is the CDN layer only)
```
...
- [ ] `SKIP_HTML_MINIFICATION: true` → HTML minification skipped entirely, reduces build time; `false` → minification runs during build (slower)
...
- [ ] Snapshots stored in Adobe Cloud infrastructure (NOT Fastly — Fastly is the CDN layer only)
