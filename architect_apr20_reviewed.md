...
```bash
# Regenerate flat tables via CLI (indexer IDs from module-catalog/etc/indexer.xml)
...
| **Declaration** | `DataPatch` + `setup:upgrade` | `extension_attributes.xml` + `db_schema.xml` |
...
In **Magento EE (Commerce)** — added by `module-catalog-staging`:

```sql
-- EE: row_id is the PK (one row per staging version)
--     entity_id is the BUSINESS identifier (shared across versions)
catalog_product_entity:
  row_id     INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,  -- physical row (identity=true)
  entity_id  INT UNSIGNED NOT NULL,                    -- logical product (identity=false)
  created_in BIGINT UNSIGNED NOT NULL DEFAULT 1,       -- staging update start timestamp
  updated_in BIGINT UNSIGNED NOT NULL DEFAULT 2147483647  -- staging update end timestamp
```

> **Exact values from module-catalog-staging/etc/db_schema.xml:** `created_in` default=1, `updated_in` default=2147483647.
...
| 3      | 1         | 1717200000      | 2147483647         |  <- scheduled update 2
+--------+-----------+------------------+--------------------+
  ^
  row_id differs, entity_id is always "1" for this product

catalog_product_entity_varchar (EE — value tables join on row_id, not entity_id)
+----------+--------+----------+--------------+-----------+
| value_id | row_id | store_id | attribute_id | value     |
+----------+--------+----------+--------------+-----------+
| 1        | 1      | 0        | 73           | 'Old Name'|
| 2        | 2      | 0        | 73           | 'New Name'|  <- staged version
+----------+--------+----------+--------------+-----------+
```

> **Note:** In EE, `entity_id` column in the value tables is `disabled="true"` (removed from the active schema) and replaced by `row_id`. The physical column may still exist during migration (`onCreate="migrateDataFrom(entity_id)"`), but logically `row_id` is the join column.
...
| Value tables join column | `entity_id` | `row_id` (entity_id disabled) |
| Safe join column | `entity_id` | `row_id` (use MetadataPool) |
| Added by module | — | `module-catalog-staging` |
...
### The Patch Interface Hierarchy

```
DependentPatchInterface
  └── getDependencies() [static]  <- ordering contract

PatchInterface extends DependentPatchInterface
  └── apply()         <- run the patch
  └── getAliases()    <- old names this replaces

DataPatchInterface extends PatchInterface
  (empty — inherits all methods above)

SchemaPatchInterface extends PatchInterface
  (empty — inherits all methods above)

PatchRevertableInterface (separate — not in hierarchy)
  └── revert()        <- rollback (optional)
```

> **Exam focus:** `DataPatchInterface` itself is **empty** — it just marks the class as a data patch (vs. schema patch). All methods come from parent interfaces. Know which methods are static vs instance:
> - `getDependencies()` — **static** (on `DependentPatchInterface`)
> - `apply()` — instance (on `PatchInterface`)
> - `getAliases()` — instance (on `PatchInterface`)
> - `revert()` — instance (on `PatchRevertableInterface`)

### The DataPatchInterface Contract

```php
<?php
// Interface hierarchy (simplified for study):
namespace Magento\Framework\Setup\Patch;

// DependentPatchInterface — ordering contract
interface DependentPatchInterface
{
    public static function getDependencies();  // no PHP return type hint
}

// PatchInterface extends DependentPatchInterface
interface PatchInterface extends DependentPatchInterface
{
    public function getAliases();  // no PHP return type hint
    public function apply();       // no PHP return type hint
}

// DataPatchInterface — empty, just marks the class as a data patch
interface DataPatchInterface extends PatchInterface
{
}

// PatchRevertableInterface — optional rollback contract
interface PatchRevertableInterface
{
    public function revert();  // no PHP return type hint
}
```

> **Exam focus:** No PHP return type hints on any interface method — consistent with Magento's `@api` interface pattern.
...
-- Table: patch_list, column: patch_name (PatchHistory::TABLE_NAME, PatchHistory::CLASS_NAME)
...
**Correct architect answer:**
> Yes, it will run again if not handled. Magento tracks patches by **FQCN (fully-qualified class name)** in the `patch_list` table's `patch_name` column. Renaming `OldPatch` to `NewPatch` makes Magento treat `NewPatch` as never-run. To prevent double-execution, add the old FQCN to `getAliases()` in the renamed class. This is critical when refactoring patch namespaces.
...
- [ ] Indexer IDs: `catalog_product_flat`, `catalog_category_flat` (module-catalog/etc/indexer.xml)
...
- [ ] Declared in `extension_attributes.xml` (XSD: `urn:magento:framework:Api/etc/extension_attributes.xsd`)
...
### EE Staging Trap
- [ ] EE staging added by `module-catalog-staging` — modifies `catalog_product_entity` via declarative schema
- [ ] EE adds `row_id` (INT UNSIGNED, `identity=true`, new PK) and disables old PK on `entity_id`
- [ ] `created_in` and `updated_in` are **BIGINT UNSIGNED** (not INT); `updated_in` default=2147483647
...
- [ ] EAV value tables in EE: `entity_id` is disabled, `row_id` becomes the FK column
- [ ] Use `MetadataPool->getMetadata(ProductInterface::class)->getLinkField()` to get correct join field
- [ ] `getLinkField()` returns `row_id` in EE, `entity_id` in CE; `MetadataPool` at `Magento\Framework\EntityManager\MetadataPool`

### Declarative Schema
- [ ] `db_schema.xml` is **state-based** — declare desired end state, Magento diffs
- [ ] XSD: `urn:magento:framework:Setup/Declaration/Schema/etc/schema.xsd`
...
### Data Patches
- [ ] `DataPatchInterface extends PatchInterface extends DependentPatchInterface` — `DataPatchInterface` itself is empty
- [ ] Implement `DataPatchInterface` — located in `Setup/Patch/Data/`
- [ ] Tracked by **FQCN** in `patch_list` table (`patch_name` column) — runs **exactly once**
- [ ] `apply()` — **instance** method, no PHP return type hint
- [ ] `getDependencies()` — **static** method (on `DependentPatchInterface`), no PHP return type hint
- [ ] `getAliases()` — **instance** method (on `PatchInterface`), no PHP return type hint
