...
class ProductPlugin
{
    ...
    // Returns null to leave args unchanged, or array to replace args
    public function beforeGetPrice(\Magento\Catalog\Model\Product $subject): ?array
    {
        return null; // leave args unchanged
    }
...
- The same method signatures (no additional required parameters)
...
// NOTE: @api service contract interfaces use NO PHP type hints — this is by design
// Actual signature: getById($productId, $editMode = false, $storeId = null, $forceReload = false)
...
**Important:** `@api` interfaces in Adobe Commerce intentionally omit PHP type hints on parameters and return types — type contracts are expressed only in docblocks. This maintains backwards compatibility and is not an LSP violation.

- **Exam focus:** LSP failures in Adobe Commerce manifest as implementations that throw unexpected exceptions, return incompatible types, or add mandatory constructor dependencies that change behavior.
...
// Full namespace: Magento\Framework\View\Element\Block\ArgumentInterface
...
class ProductRepositoryTest extends \PHPUnit\Framework\TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->repository = \Magento\TestFramework\Helper\Bootstrap::getObjectManager()->get(
...
**Note on integration test base classes:**
- **Non-controller integration tests** extend `\PHPUnit\Framework\TestCase` directly
- **Controller integration tests** extend `\Magento\TestFramework\TestCase\AbstractController` (adds `dispatch()`, `_request`, `_response` helpers)
- **Admin controller integration tests** extend `\Magento\TestFramework\TestCase\AbstractBackendController`
- Annotations (`@magentoDataFixture`, etc.) are processed by PHPUnit listeners regardless of which base class you use
...
- Uses a **real test database** (`magento_integration_tests` — configured in `dev/tests/integration/etc/install-config-mysql.php`)
- Slower, but catches wiring/integration problems unit tests can't
- Lives in `dev/tests/integration/`

**Base class selection (important for exam):**

| Test type | Correct base class |
|---|---|
| General integration (repository, service, model) | `\PHPUnit\Framework\TestCase` |
| Frontend controller dispatch | `\Magento\TestFramework\TestCase\AbstractController` |
| Admin controller dispatch | `\Magento\TestFramework\TestCase\AbstractBackendController` |

```php
namespace Vendor\Module\Test\Integration\Model;

// General integration test — extend PHPUnit TestCase directly
class ProductRepositoryTest extends \PHPUnit\Framework\TestCase
{
    private \Magento\Catalog\Api\ProductRepositoryInterface $repository;
...
- **Exam focus:** The integration test DB is `magento_integration_tests` — a separate database configured in `install-config-mysql.php`.
- **Exam focus:** Non-controller integration tests should extend `\PHPUnit\Framework\TestCase`, not `AbstractController`. `AbstractController` is specifically for testing controller dispatch flow.
...
**Suite** — grouping tests with shared setup/teardown:
...
```bash
# Generate all tests from XML (required before first run)
vendor/bin/mftf generate:tests

# Generate a specific suite
vendor/bin/mftf generate:suite CheckoutSuite

# Run a single test (generates + executes)
vendor/bin/mftf run:test AddSimpleProductToCartTest

# Run a group (by annotation group value)
vendor/bin/mftf run:group catalog

# Re-run failed tests
vendor/bin/mftf run:failed

# Generate and remove cached files first
vendor/bin/mftf run:test --remove AddSimpleProductToCartTest
```

**Important:** There is no `run:suite` command in MFTF. To run a suite: use `generate:suite <SuiteName>` to generate the suite's test files, then run the generated tests via `run:group` or directly with PHPUnit.
...
- **Exam focus:** `mftf run:test` runs a single test by name. `mftf run:group` runs by group annotation. Know this command syntax.
- **Exam focus:** MFTF lives in `dev/tests/acceptance/` at the project level for core tests; module-level tests live in `Vendor/Module/Test/Mftf/`.
...
- Uses `@magentoApiDataFixture` (note: NOT `@magentoDataFixture`)
...
- **Exam focus:** API functional tests use `@magentoApiDataFixture`, not `@magentoDataFixture`.
...
- Full interface: Magento\Framework\View\Element\Block\ArgumentInterface
```
...
- [ ] **LSP**: `@api` interfaces have NO PHP type hints — contracts are expressed in docblocks only.
...
- [ ] **Forbidden**: Business logic in templates — use ViewModel implementing `Magento\Framework\View\Element\Block\ArgumentInterface`.
...
- [ ] **General tests**: extend `\PHPUnit\Framework\TestCase` directly
- [ ] **Controller tests**: extend `\Magento\TestFramework\TestCase\AbstractController`
- [ ] **Admin controller tests**: extend `\Magento\TestFramework\TestCase\AbstractBackendController`
- [ ] Uses real DI container via `\Magento\TestFramework\Helper\Bootstrap::getObjectManager()`
- [ ] Uses real DB (`magento_integration_tests` — configured in `install-config-mysql.php`)
...
- [ ] Command: `vendor/bin/mftf run:test TestName` — generate + execute
- [ ] Command: `vendor/bin/mftf run:group GroupName` — run by group annotation
- [ ] Command: `vendor/bin/mftf generate:suite SuiteName` — generate suite (no `run:suite` command exists)
- [ ] Module tests live in `Vendor/Module/Test/Mftf/`; core tests in `dev/tests/acceptance/`
...
- [ ] Uses `@magentoApiDataFixture` (NOT `@magentoDataFixture`)
- [ ] Lives in `dev/tests/api-functional/`
...
- [ ] **Integration test for repository — which base class?** → `\PHPUnit\Framework\TestCase`
- [ ] **MFTF run suite?** → `generate:suite` then `run:group` (no `run:suite` command)
