...
- Additionally removes `javascript:`, `vbscript:`, **and `data:`** protocol schemes to prevent protocol-based XSS (confirmed in `Magento\Framework\Escaper::escapeScriptIdentifiers()`)
...
**Why it blocks `javascript:` and `data:` attacks:**
...
> **Exam focus:** `escapeUrl()` is specifically designed to **prevent protocol injection** — it strips `javascript:`, `vbscript:`, and `data:` schemes. Use it for ALL URL values in HTML attributes. Do NOT use `escapeHtml()` alone for URLs.
...
- [ ] `escapeUrl($val)` → URL attributes: `href=""`, `src=""`, `action=""` — strips `javascript:`, `vbscript:`, and `data:` schemes
...
- [ ] Use `escapeUrl()` (not `escapeHtml()`) for URL attributes — escapeHtml alone won't strip `javascript:`, `vbscript:`, or `data:` schemes
...
- [ ] `escapeUrl()` specifically prevents protocol injection — strips `javascript:`, `vbscript:`, and `data:` schemes — no other escape method does this
