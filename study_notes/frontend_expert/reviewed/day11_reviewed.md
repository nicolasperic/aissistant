...
> **Important:** This `TargetComponent.extend()` pattern works for **UI Components** (modules that extend `uiClass` or `uiComponent`). For jQuery widgets like `Magento_Catalog/js/price-box`, use the `$.widget()` approach shown in Section 5. `price-box` returns `$.mage.priceBox` (a jQuery widget prototype) which does not have `.extend()`.
...
> **Important:** `Magento_Catalog/js/price-box` is a **jQuery widget** (it registers `$.widget('mage.priceBox', {...})` and returns `$.mage.priceBox`). It does NOT use the `uiComponent`/`uiClass` inheritance chain. Therefore, the correct mixin approach for price-box is the **widget mixin** using `$.widget()` — not `TargetComponent.extend()`. For exam purposes: when the study notes show `TargetComponent.extend()` as the pattern, that applies to UI Components; when the target is a jQuery widget, use `$.widget('mage.widgetName', $.mage.widgetName, {...})`.
...
<config xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    ...
    <module name="Vendor_PriceMixin"/>
</config>
```

> **Correction from original notes:** `setup_version` is **deprecated and removed** in Magento 2.4.x. Do not include it. Modern `module.xml` uses only `<module name="..."/>` with optional `<sequence>` children.
...
Since `price-box` is a jQuery widget, the correct mixin uses `$.widget()`:

```javascript
/**
 * Mixin for Magento_Catalog/js/price-box (a jQuery widget)
 *
 * Demonstrates:
 * 1. Correct widget mixin function signature (no argument)
 * 2. Method extension using this._super()
 * 3. Overriding an existing widget method (reloadPrice)
 */
define([
    'jquery'
], function ($) {
    ...
    return function () {
        ...
        $.widget('mage.priceBox', $.mage.priceBox, {
            ...
            /**
             * Override _create (widget init hook — equivalent of initialize for widgets).
             * _create() runs when the widget first initialises on an element.
             */
            _create: function () {
                // Run original _create first
                this._super();

                // Then add our custom initialisation
                console.log('[PriceMixin] Price box created with options:', this.options);
            }
...
> **Note:** jQuery widgets use `_create` (and optionally `_init`) as lifecycle hooks, not `initialize`. The `initialize` method belongs to the UI Component (`uiClass`) inheritance chain.
...
```
[PriceMixin] Price box created with options: {priceConfig: {...}}
```
...
### Mistake 7 — Using `TargetComponent.extend()` on a jQuery Widget

```javascript
// WRONG — price-box is a jQuery widget; it returns $.mage.priceBox
// which does not have an .extend() method
return function (TargetComponent) {
    return TargetComponent.extend({ ... });  // TypeError!
};

// CORRECT for jQuery widget targets
return function () {
    $.widget('mage.priceBox', $.mage.priceBox, { ... });
};
```
...
File on disk (core modules live in `vendor/`, not `app/code/`):
```
vendor/magento/module-catalog/view/base/web/js/price-box.js
```

> **Correction:** Core Magento modules live under `vendor/magento/module-{name}/`, not `app/code/Magento/`. Also note that `price-box.js` is in `view/base/` (shared across frontend and adminhtml), not `view/frontend/`. The module ID `Magento_Catalog/js/price-box` still resolves correctly via Commerce's fallback mechanism.
...
### Mixin File Signature (Component Mixin — for UI Components)
...
- [ ] Use `initialize` as the lifecycle hook for UI Components

### Mixin File Signature (Widget Mixin — for jQuery Widgets)
...
- [ ] Use `_create` (not `initialize`) as the lifecycle hook for jQuery widgets
...
- [ ] Component mixin (UI Component): `function(Target) { return Target.extend({}) }`
- [ ] Widget mixin (jQuery widget): `function() { $.widget(...); }` (no argument)
...
- [ ] Core modules live under `vendor/magento/module-{name}/`, not `app/code/Magento/`
...
- [ ] Do NOT use `TargetComponent.extend()` for jQuery widget targets — use `$.widget()` instead
- [ ] Do NOT include `setup_version` in `module.xml` for Magento 2.4.x
- [ ] Do NOT use `initialize` as the lifecycle method for jQuery widgets — use `_create` or `_init`
