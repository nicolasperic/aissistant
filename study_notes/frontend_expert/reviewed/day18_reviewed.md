...
#### Step 1: package.json.sample

Commerce ships a `package.json.sample` file at the root — it is a sample that must be copied to `package.json` before running `npm install` (just like `Gruntfile.js.sample`). It declares Node.js dependencies (Grunt plugins). Think of it as the `composer.json` equivalent for Node.

```bash
cp package.json.sample package.json
```

Key packages in `devDependencies` (from `package.json.sample`):
...
| `grunt-contrib-jasmine` | Runs Jasmine unit tests |
| `grunt-exec` | Runs arbitrary shell commands from Grunt |

> Note: `grunt-contrib-uglify` is **not** in Commerce's `package.json.sample`. JS minification is handled separately.
...
Commerce provides a `Gruntfile.js` that orchestrates all the tasks. There are also sample files — you copy both samples before starting:
...
# 2. Check if package.json exists (Commerce ships package.json.sample — copy it first)
ls -la package.json.sample
cp package.json.sample package.json
...
```bash
# Edit a LESS variable (luma is in vendor/, not app/design/)
vim vendor/magento/theme-frontend-luma/web/css/source/_variables.less
...
- [ ] `package.json.sample` declares Grunt plugin dependencies — copy to `package.json` before `npm install`
- [ ] `Gruntfile.js` orchestrates tasks; copy from `Gruntfile.js.sample` to start (also copy `package.json.sample`)
