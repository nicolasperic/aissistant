...
> **Note:** The actual method is `getCopyright()` (defined in `Magento\Theme\Block\Html\Footer`), NOT `getCopyrightInfo()`. The outer wrapper is `<div class="footer-container">`, not `<footer class="page-footer">`. The template also includes the "Help Us Keep Magento Healthy" / "Report All Bugs" section — a common source of confusion in exam scenarios.
...
> **Note:** Use `getCopyright()` (not `getCopyrightInfo()`). Match the actual outer wrapper `<div class="footer-container">` to avoid layout conflicts with module CSS that targets `.footer-container`.
