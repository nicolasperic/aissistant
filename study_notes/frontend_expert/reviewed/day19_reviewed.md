...
## Adobe Commerce Front-End Developer Expert (AD0-E727) — Comprehensive Study Notes
...
- `preview.jpg` must be exactly **800×800 px** at `<theme>/media/preview.jpg` *(verified: both blank and luma ship 800×800 px JPEG)*
...
- `<block>` `class` attribute is **optional** — it defaults to `Magento\Framework\View\Element\Template` when omitted; only `name` is effectively required in practice *(XSD: `use="optional"`)*
...
<!-- Preferred modern approach: remove="true" attribute on <referenceBlock> -->
```

**Exam focus:**
- The preferred way to remove a block is `remove="true"` on `<referenceBlock>`
- A legacy `<remove>` element **does exist** in `elements.xsd` (line 73) but is not the recommended modern approach — exam questions almost always refer to `remove="true"` on `<referenceBlock>` *(verified: `elements.xsd:73`)*
...
lib/web/css/source/
  lib/
    ...
    _icons.less
    _layout.less
    _loaders.less         # NOTE: _list.less does NOT exist in lib — removed
    _messages.less
    _navigation.less
    _pages.less
    _popups.less
...
- Variable override works because LESS uses **lazy variable evaluation** — the last declaration of a variable wins at compile time, regardless of import order. In the blank theme, `_theme.less` is actually imported **after** the library components, yet variables declared in it still override the defaults *(LESS lazy eval, not import order)*
...
- **Developer mode** uses **server-side PHP (less.php)** compilation — NOT client-side `less.js` *(verified: Day 8)*
- Client-side `less.js` compilation requires an **explicit config** setting (`dev/front_end_development_workflow/type` = `client_side_compilation`), it does not activate automatically with developer mode
...
define([
    ...
    'Magento_Ui/js/lib/core/element/element',   // element base (file exists; use 'uiComponent' alias → collection for typical UI component work)
    'ko'                                         // Knockout.js
...
define([
    'uiComponent',          // Base: Magento_Ui/js/lib/core/collection (NOT element/element)
    'ko'
...
**Exam focus:**
- UI components extend `uiComponent` (alias for `Magento_Ui/js/lib/core/collection`) *(verified: module-ui requirejs-config.js:28)*
...
```xml
<!-- Vendor_Module/view/adminhtml/pagebuilder/content_type/my_type.xml -->
<!-- NOTE: path is view/adminhtml/pagebuilder/, NOT etc/adminhtml/pagebuilder/ -->
...
|-------------|---------------|----------------|---------|
| `<block>` | `name` | `class` *(optional, defaults to Template)*, `template`, `before`, `after`, `as`, `cacheable`, `aclResource` | Create a new block |
...
- [ ] `preview.jpg` is **800×800 px** at `media/preview.jpg` *(verified: blank and luma ship 800×800)*
...
- [ ] `<block>` `name` is required; `class` is **optional** (defaults to `Magento\Framework\View\Element\Template`); `<container>` has no class/template
