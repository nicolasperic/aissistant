...
|  OR                                           |
|  mage/apply/main.js      (widgets/simple)     |
+-----------------------------------------------+
```
...
Page loads
    |
    v
mage/apply/main.js scans DOM for [data-mage-init]
(mage/apply/scripts.js separately processes x-magento-init script tags)
    |
    v
...
The base class `Magento\Framework\View\Element\AbstractBlock` defines `getJsLayout()` (inherited by `Template`):
...
In the template (e.g., `onepage.phtml`):
...
| **When parsed** | Once, on DOMContentLoaded | Each time `apply()` is called |
...
**Exam focus:** `mage/apply/main` must be called explicitly (via `mage.apply()`) after dynamically adding HTML that contains `data-mage-init` attributes. Static initialization happens automatically on page load.
...
class CustomWidget extends Template
{
    ...
    // getJsLayout() is inherited from AbstractBlock (via Template) — it reads the jsLayout argument
...
BROWSER - HTML RENDERED
    |
    | mage/apply/main.js scans [data-mage-init] attributes
    | mage/apply/scripts.js processes x-magento-init script tags
    |
    v
...
### Mistake 6: Misconception about where `getJsLayout()` comes from

`getJsLayout()` is defined in `Magento\Framework\View\Element\AbstractBlock` — **not** in `Template`. Since `Template extends AbstractBlock`, any block that extends either class gets `getJsLayout()` automatically.

```php
// AbstractBlock defines getJsLayout() — both of these get it:
class MyBlock extends \Magento\Framework\View\Element\Template {}       // ✓ inherits via Template
class MyBlock extends \Magento\Framework\View\Element\AbstractBlock {}  // ✓ inherits directly
```

The method only would be missing if you extend a class that does not descend from `AbstractBlock` at all (extremely rare in Magento).

**Exam focus:** `jsLayout` is processed by `AbstractBlock`, which is the base for virtually all Magento block classes including `Template`.
...
- [ ] Parsed automatically on page load by `mage/apply/main.js` (which also invokes `mage/apply/scripts.js` for `x-magento-init`)
- [ ] For dynamically injected HTML, call `mage.apply()` (require `mage/apply/main`) manually
...
- [ ] `getJsLayout()` is defined in `Magento\Framework\View\Element\AbstractBlock` (inherited by `Template`)
...
- [ ] **Dynamic**: `mage.apply()` (require `mage/apply/main`) called after injecting new HTML into the DOM
