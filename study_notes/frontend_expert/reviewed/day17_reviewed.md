...
- Each content type is registered via its own **XML configuration file** in `view/adminhtml/pagebuilder/content_type/`
...
|---|---|---|
| `<name>.xml` | Registers the type with Page Builder | `view/adminhtml/pagebuilder/content_type/` |
| `preview.html` | Knockout template shown in Admin stage | `view/adminhtml/web/template/content-type/<name>/default/` |
| `master.html` | HTML template output to storefront | `view/adminhtml/web/template/content-type/<name>/default/` |
...
Each content type has its **own dedicated XML file** at `view/adminhtml/pagebuilder/content_type/<name>.xml`. There is no single monolithic `content_types.xml` — each type lives in a separate file.

```xml
<!-- view/adminhtml/pagebuilder/content_type/my_quote.xml -->
<?xml version="1.0"?>
...
| `component` | Base RequireJS component (leaf types use `Magento_PageBuilder/js/content-type`) |
| `preview_component` | RequireJS path to the per-type Knockout ViewModel JS file |
| `form` | UI Component form name (e.g., `my_quote_form`) shown in the settings panel |
| `menu_section` | Which panel group the type appears in (`layout`, `elements`, `media`, `add_content`) |
...
| Dynamic Block | `dynamic_block` | Add Content (EE — in `module-banner-page-builder`) |
| Banner | `banner` | Media |
...
Create a dedicated XML file for your content type at `view/adminhtml/pagebuilder/content_type/quote_block.xml`:

```xml
<!-- view/adminhtml/pagebuilder/content_type/quote_block.xml -->
<?xml version="1.0"?>
<config xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
        xsi:noNamespaceSchemaLocation="urn:magento:module:Magento_PageBuilder:etc/content_type.xsd">
...
You extend an existing content type via **XML merge** — place a file for that type's name in your module's `view/adminhtml/pagebuilder/content_type/` directory. Magento's XML merging applies your additions on top of the core config.

```xml
<!-- view/adminhtml/pagebuilder/content_type/heading.xml in YOUR module -->
<?xml version="1.0"?>
<config xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
        xsi:noNamespaceSchemaLocation="urn:magento:module:Magento_PageBuilder:etc/content_type.xsd">
...
> This works because each content type's XML is **merged** across modules — just like `di.xml` or `config.xml`.

**Exam focus:**
- Extending existing types uses **XML merge** — create a file with the same type name in your module's `content_type/` directory
...
```xml
<!-- In your view/adminhtml/pagebuilder/content_type/heading.xml extension -->
<type name="heading">
    <appearances>
        <appearance name="default"
...
Then wire this up in your `content_type/heading.xml` extension by setting `preview_component` to your new path:
...
- Wire the extension by setting `preview_component` in your `content_type/heading.xml` declaration
...
find vendor/magento/module-page-builder/view/adminhtml/pagebuilder/content_type \
  -name "*.xml" \
  -exec echo "=== {} ===" \; \
  -exec head -15 {} \;
```
...
grep -r 'name=' \
  vendor/magento/module-page-builder/view/adminhtml/pagebuilder/content_type/ \
  | grep '<type ' \
  | awk -F'"' '{print $2}' \
  | sort
```
...
# etc/                    - Schema definitions (XSD files: content_type.xsd, menu_section.xsd)
# view/adminhtml/pagebuilder/content_type/  - Individual XML file per content type
...
view/adminhtml/pagebuilder/content_type/<name>.xml
    ...
    |-- component           <-- base RequireJS component (generic per type class)
    |-- preview_component   <-- per-type Knockout ViewModel
    |-- form                <-- UI Component name for settings panel
    |-- appearances
...
### Page Builder — Core Concepts
- [ ] Page Builder is **EE only** — not in Magento Open Source
- [ ] Content is stored as **annotated HTML** with `data-content-type` attributes in standard DB text columns
- [ ] Built on **Knockout.js** for Admin stage; storefront renders static HTML with no JS engine
- [ ] A content type has **two templates**: preview (admin, KO-powered) and master (storefront, static)
- [ ] Each content type has its own XML file at `view/adminhtml/pagebuilder/content_type/<name>.xml`
- [ ] The XSD schema is `urn:magento:module:Magento_PageBuilder:etc/content_type.xsd` (singular)

### Page Builder — XML Config
- [ ] `component` = base RequireJS component (`Magento_PageBuilder/js/content-type` for leaf types)
- [ ] `preview_component` = per-type Knockout ViewModel path (your custom JS)
- [ ] `form` = UI component name matching the filename (e.g., `quote_block_form`)
- [ ] `menu_section` values: `layout`, `elements`, `media`, `add_content`
- [ ] `appearances > appearance` declares `preview_template` and `master_template` (RequireJS-style paths)
- [ ] `elements > element > style/attribute/css/html` maps form fields to HTML

### Page Builder — File Locations
- [ ] Content type XML: `view/adminhtml/pagebuilder/content_type/<name>.xml`
- [ ] Preview template: `view/adminhtml/web/template/content-type/<name>/default/preview.html`
- [ ] Master template: `view/adminhtml/web/template/content-type/<name>/default/master.html`
- [ ] Preview JS: `view/adminhtml/web/js/content-type/<name>/preview.js`
- [ ] Edit form: `view/adminhtml/ui_component/<name>_form.xml`

### Page Builder — Extension Pattern
- [ ] Extend via **XML merge**: create `content_type/<name>.xml` in your module with only what you change
- [ ] Extending JS: inherit from `Magento_PageBuilder/js/content-type/<name>/preview`, wire via `preview_component`
- [ ] Overriding templates: point `preview_template`/`master_template` to your module's path

### Admin UI SDK
- [ ] **Out-of-process** — runs on Adobe I/O Runtime, not Commerce server
- [ ] Extensions are **React applications**, not PHP
- [ ] Requires **App Builder entitlement** (Commerce Cloud / on-prem with subscription)
- [ ] Data access via **REST/GraphQL APIs**, not direct DB
- [ ] Config declared in `app.config.yaml` under `extensionPoints`
- [ ] Extension points: `menu`, `page`, `massAction`, `column`, `banner`, `modal`, `sharedContext`
- [ ] Lower upgrade risk than traditional PHP admin (decoupled from Commerce core)

### When to Choose Which
- [ ] Admin UI SDK → new dashboards, reporting, external data display, zero upgrade risk
- [ ] Traditional PHP → direct service layer access, complex form validation, ACL, offline scenarios
- [ ] SDK is an **additional tool**, not a full replacement for PHP admin
