...
//  (NOT vendor/magento/theme-frontend-blank/web/css/source/lib/variables/ — that path does not exist)
//  ---------------------------------------------
...
```bash
# Check theme is registered in the database (query directly)
mysql -u magento -p magento -e "SELECT * FROM theme WHERE theme_path = 'frontend/MyVendor/mytheme';"

# Or verify via Admin: Content > Design > Themes — your theme should appear in the list
```

> **Note:** `bin/magento info:dependencies:show-framework` does NOT check theme registration — it shows the number of framework dependencies per module. Use the DB query or Admin UI to confirm theme registration.
