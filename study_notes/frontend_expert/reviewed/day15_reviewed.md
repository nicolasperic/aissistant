...
$.widget('mage.accordion', {
    ...
    options: {
        active: [0],          // Array of active panel indexes ([0] = first panel open)
        multipleCollapsible: false,  // Allow multiple panels open at once
        openOnFocus: false    // Open panel on keyboard focus
        // Note: accordion inherits additional options from mage.tabs and mage.collapsible,
        // including openedState (default: null) and animate (default: false)
    },
...
> **Correction:** `mage.accordion`'s own options object only defines `active: [0]` (an **array**, not `0`), `multipleCollapsible: false`, and `openOnFocus: false`. Options like `collapsible`, `openedState`, and `animate` are inherited from `mage.collapsible` (where defaults are `collapsible: true`, `openedState: null`, `animate: false`). The `animate` default is `false`, not `{ duration: 300 }`.
...
Core widgets live under `lib/web/mage/` or `vendor/magento/module-ui/view/base/web/js/`. Here are the most important ones.

### 4.1 `mage.accordion`

**File:** `lib/web/mage/accordion.js`
**Extends:** `mage.tabs` (which extends `mage.collapsible`)

```javascript
// mage.accordion's own options (inherits more from mage.tabs and mage.collapsible)
{
    active: [0],              // ARRAY — indexes of active panels (not a single number)
    multipleCollapsible: false, // Allow multiple panels open simultaneously
    openOnFocus: false
    // Inherited from mage.collapsible: collapsible: true, openedState: null, animate: false
}
```
...
### 4.3 `mage.dropdownDialog`

**File:** `lib/web/mage/dropdown.js`
**Widget name:** `mage.dropdownDialog` (NOT `mage.dropdown`)
**RequireJS alias:** `'dropdownDialog'` maps to `'mage/dropdown'`

> **Important:** The widget defined in `lib/web/mage/dropdown.js` is `$.widget('mage.dropdownDialog', $.ui.dialog, {...})` — it extends jQuery UI Dialog. There is **no** `mage.dropdown` widget. The short alias `'dropdown'` actually maps to `'mage/dropdowns'` (a different file).

```javascript
// Key options for mage.dropdownDialog
{
    triggerEvent: 'click',          // Event to open the dialog
    triggerTarget: null,            // Selector of the trigger element
    triggerClass: null,             // Class added to trigger when open
    parentClass: null,              // Class added to parent element when open
    defaultDialogClass: 'mage-dropdown-dialog',
    closeOnMouseLeave: true,        // Close when mouse leaves the dialog
    closeOnClickOutside: true,      // Close when clicking outside
    autoOpen: false,
    bodyClass: '',                  // Class added to body when open
    modal: false
}
```
...
**File:** `vendor/magento/module-ui/view/base/web/js/modal/modal.js`
**RequireJS module ID:** `Magento_Ui/js/modal/modal`
**Note:** There is NO `lib/web/mage/modal.js`.

```javascript
// Key options (verified against actual codebase)
{
    type: 'popup',          // 'popup', 'slide', 'custom'
    title: '',              // Modal title
    modalClass: '',         // Additional CSS class
    buttons: [...],         // Array of button config objects (has default close button)
    responsive: false,      // NOTE: default is false, not true
    innerScroll: false,
...
```html
<!-- Basic modal example — use Magento_Ui/js/modal/modal as the module ID -->
<div data-mage-init='{
...
**RequireJS alias:** `'loader'` → `'mage/loader'`
...
- `mage.modal` is in `Magento_Ui/js/modal/modal` — **not** in `lib/web/mage/`.
...
<script type="text/x-magento-init">
{
    "#my-dropdown": {
        "mage/dropdown": {
            ...
            "triggerTarget": ".dropdown-trigger"
        }
    }
}
</script>
...
### Real-World Example: Extending `mage.dropdownDialog`

```javascript
// File: Vendor_Module/view/frontend/web/js/my-dropdown.js
define([
    'jquery',
    'mage/dropdown'   // <-- loads and registers $.mage.dropdownDialog
], function ($) {
    'use strict';

    // The widget in mage/dropdown is mage.dropdownDialog (extends $.ui.dialog)
    $.widget('vendorModule.myDropdown', $.mage.dropdownDialog, {

        // Extend default options
        options: {
            closeOnMouseLeave: false,   // override parent default
...
**Exam focus:**
- The parent widget reference uses the **full dotted name**: `$.mage.accordion`, `$.mage.dropdownDialog`, `$.mage.modal`.
- The widget in `mage/dropdown` is `mage.dropdownDialog` — **not** `mage.dropdown`.
...
> **Note:** The `trigger` option binds an external button to open the modal. The widget initializes on `#my-modal-container` but the trigger is `#open-my-modal`. The `responsive` default is `false`.

### Example 2: Custom Widget Extending `mage.dropdownDialog`
...
define([
    ...
    /**
     * Custom dropdown widget that extends mage.dropdownDialog
     * Note: lib/web/mage/dropdown.js defines $.widget('mage.dropdownDialog', $.ui.dialog, {...})
     */
    $.widget('vendorModule.vendorDropdown', $.mage.dropdownDialog, {
         ...
         */
        _create: function () {
            // Run parent setup first (binds core dropdownDialog events)
            this._super();
...
RequireJS loads and executes
        |
        v
'mage/bootstrap' is auto-required via deps in Magento_Theme's requirejs-config.js
        |
        v
...
> **Correction:** `mage/bootstrap` is NOT loaded from layout XML. It is loaded automatically because it is listed in the `deps` array of `Magento_Theme/view/frontend/requirejs-config.js`. This file is processed by RequireJS when the page loads — no explicit `<script>` or layout XML reference needed.

### RequireJS Module Path Mapping

Magento maps short module paths to full file paths in `requirejs-config.js`. The actual aliases defined in `Magento_Theme/view/frontend/requirejs-config.js` include:
...
var config = {
    map: {
        '*': {
            ...
            'dropdownDialog': 'mage/dropdown',    // mage.dropdownDialog widget
            'dropdown':       'mage/dropdowns',   // different file — NOT mage/dropdown
            'loader':         'mage/loader',
            'tooltip':        'mage/tooltip',
            'menu':           'mage/menu',
            'validation':     'mage/validation/validation'
            // NOTE: there is NO short alias for 'modal' — use full path: Magento_Ui/js/modal/modal
        }
    },
    deps: [
        'mage/common',
        'mage/dataPost',
        'mage/bootstrap'    // auto-loaded here — no layout XML needed
    ]
};
```
...
- There is **no** short alias `'modal'` — always use `'Magento_Ui/js/modal/modal'` as the full path.
- The alias `'dropdownDialog'` → `'mage/dropdown'`; the alias `'dropdown'` → `'mage/dropdowns'` (different file).
...
### Mistake 6: Confusing `mage.dropdown` and `mage.dropdownDialog`

```javascript
// WRONG: $.mage.dropdown does not exist
$.widget('vendor.myDropdown', $.mage.dropdown, { ... });

// CORRECT: the widget in mage/dropdown is mage.dropdownDialog
$.widget('vendor.myDropdown', $.mage.dropdownDialog, { ... });
```

```html
<!-- WRONG: wrong module path assumption -->
<div data-mage-init='{"mage/dropdown": {"autoclose": true}}'>
    <!-- mage/dropdown has no 'autoclose' option -->

<!-- mage/dropdown is dropdownDialog — use its actual options -->
<div data-mage-init='{"mage/dropdown": {"closeOnMouseLeave": true, "triggerTarget": ".my-trigger"}}'>
```
...
- [ ] `mage/accordion` → `$.widget('mage.accordion', tabs, {...})` — `active: [0]` (array), `multipleCollapsible: false`
- [ ] `mage/tabs` → `$.widget('mage.tabs', ...)` — tabbed interface, similar data-role structure
- [ ] `mage/dropdown` → `$.widget('mage.dropdownDialog', $.ui.dialog, {...})` — NOT `mage.dropdown`
- [ ] `Magento_Ui/js/modal/modal` → `$.widget('mage.modal', {...})` — types: `popup`, `slide`, `custom`; file in `vendor/magento/module-ui/`
...
- [ ] Access parent accordion: `$.mage.accordion`; parent dialog-dropdown: `$.mage.dropdownDialog`
...
### RequireJS Aliases (Magento_Theme/view/frontend/requirejs-config.js)

- [ ] `'accordion'` → `'mage/accordion'` ✓
- [ ] `'tabs'` → `'mage/tabs'` ✓
- [ ] `'collapsible'` → `'mage/collapsible'` ✓
- [ ] `'dropdownDialog'` → `'mage/dropdown'` (the dropdownDialog widget)
- [ ] `'dropdown'` → `'mage/dropdowns'` (different file — NOT `mage/dropdown`)
- [ ] `'loader'` → `'mage/loader'` ✓
- [ ] **NO** `'modal'` alias — always use full path `'Magento_Ui/js/modal/modal'`
- [ ] `mage/bootstrap` auto-loaded via `deps` in requirejs-config.js — NOT from layout XML
...
- [ ] `mage.modal` is in `Magento_Ui/js/modal/modal` — **not** `lib/web/mage/modal.js`
- [ ] The widget in `mage/dropdown` is `mage.dropdownDialog` — **not** `mage.dropdown`
- [ ] `mage.accordion`'s `active` default is `[0]` (array) — **not** `0` (number)
- [ ] `modal` default `responsive` is `false` — **not** `true`
