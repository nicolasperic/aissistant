...
| **Client-side (Less.js)** | Manually enabled via config (`dev/front_end_development_workflow/type client_side_compilation`) | Browser memory | Debugging only |
| **Grunt (local)** | `grunt watch` / `grunt less` | `pub/static/` | Development workflow |

**Exam focus:** In **developer mode**, LESS is compiled **server-side by the PHP less.php library** (`static.php` handles the request) — NOT by `less.js` in the browser. Client-side compilation is a separate mode that must be explicitly enabled via `bin/magento config:set dev/front_end_development_workflow/type client_side_compilation`. In **production mode**, LESS must be pre-compiled via `setup:static-content:deploy` or Grunt.
...
```less
// styles-m.less  (compiled entry point) — actual blank theme import order:
@import 'source/_reset.less';
@import '_styles.less';                    // → lib/_lib.less, _sources.less, _components.less
//@magento_import 'source/_module.less';  // per-module styles collected here
//@magento_import 'source/_widgets.less';
@import 'source/_theme.less';             // YOUR variable overrides (loaded AFTER lib vars)
//@magento_import 'source/_extend.less';
@import 'source/lib/_responsive.less';
```

> `_variables.less`, `_typography.less`, and `_buttons.less` are NOT directly imported by `styles-m.less` — they are included indirectly via `_styles.less` → `lib/_lib.less`. See Day 13 verified facts.
...
// Component shortcuts
.lib-button();
.lib-form-element-input();  // Note: .lib-input-style() does NOT exist; use .lib-form-element-input()
.lib-dropdown();
.lib-clearfix();
```
...
define([
    'uiComponent',    // alias for Magento_Ui/js/lib/core/collection (NOT /component — both uiComponent and uiCollection map to collection.js)
    'ko'
...
- [ ] In **developer mode**, LESS is compiled **server-side by PHP** (`less.php` library) on each request — NOT by `less.js` in the browser
- [ ] In **production mode**, LESS must be pre-compiled via `setup:static-content:deploy` or Grunt
