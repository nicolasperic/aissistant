...
> **Exam focus:** The Adobe AD0-E721 exam tests *application* of knowledge, not memorization. Every concept below has appeared in scenario-based questions. Ask yourself *"What would break if this were wrong?"* for each item.
...
| Has a PHP class | NO (always `AbstractBlock`) | YES (custom class) |
...
> **Exam focus:** The passing score for AD0-E721 is typically **68%** (Adobe adjusts this periodically). A baseline of 50–60% with 2 weeks to go is entirely normal and recoverable.
...
**Answer: B**
*The template engine (`Magento\Framework\View\TemplateEngine\Php`) extracts several variables into the template scope, including `$escaper`, `$block`, and `$templateContext`.*
...
**Answer: B**
*`mage/translate` is the RequireJS module that provides the `$t()` translation function in Magento JavaScript. It maps to `lib/web/mage/translate.js`.*
...
- [ ] JS translations use `mage/translate` RequireJS module (provides `$t()`)
