...
Sales module dispatches event
     |
     v
Magento_Email module TransportBuilder
     |
     v
Resolves template:
...
## 7. Email Layout Handles

### 7.1 Email Templates and Layout XML

Unlike frontend pages, email templates are **not** rendered through the full layout XML pipeline. However, Magento does use a simplified layout handle system for email.

Email-specific layout handles are defined in:

```
app/design/frontend/<Vendor>/<theme>/Magento_Email/layout/
  email_default.xml     <-- applies to all emails
  email_template.xml    <-- per-template handle (rare)
```

### 7.2 The Email Styles Block

The primary use of email layout handles is to declare which CSS files are loaded and inlined:

```xml
<!-- app/design/frontend/MyVendor/mytheme/Magento_Email/layout/email_default.xml -->
<?xml version="1.0"?>
<page xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
      xsi:noNamespaceSchemaLocation="urn:magento:framework:View/Layout/etc/page_configuration.xsd">
    <head>
        <css src="css/email-inline.css"/>
        <css src="css/email.css"/>
    </head>
</page>
```

> **Exam focus:** Email templates do NOT use the same full layout XML handles as storefront pages. The email layout handle `email_default` is distinct from storefront handles like `default` or `catalog_product_view`.
...
- [ ] Email templates do NOT use the full storefront layout XML pipeline
- [ ] Email layout handle: `email_default` (in `Magento_Email/layout/`)
