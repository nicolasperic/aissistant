...
| `head.additional` | `<head>` additions |
...
In **page layout files** (1column.xml, 2columns-left.xml, etc.) and in **full page layout files** used in themes, the root element is `<page>`.
...
<page_layouts xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
              xsi:noNamespaceSchemaLocation="urn:magento:framework:View/Layout/etc/layouts.xsd">
...
```bash
# Verify layout XML merges correctly (shows final merged layout)
bin/magento dev:layout:xml --area=frontend --theme=Vendor/Theme --handle=cms_index_index
...
- [ ] `head.additional` — additional `<head>` elements
