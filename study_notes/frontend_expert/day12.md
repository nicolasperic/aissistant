...
Every KO-based component in Commerce extends `Magento_Ui/js/lib/core/collection` (aliased as `uiComponent`).
...
```javascript
// Simplified version of quote.js
define(['ko'], function(ko) {
    'use strict';
    ...
    var totals             = ko.observable(null);
    var isVirtual          = ko.observable(false);
    ...
    return {
        ...
        // When shippingMethod changes, totals component auto-updates
        // because totals.js subscribes to shippingMethod
    };
});
```
...
```javascript
// Simplified version of Magento_Checkout/js/view/form/shipping.js
define([
    'uiComponent',
    ...
    return Component.extend({
        defaults: {
            template: 'Magento_Checkout/form/shipping'
        },
...
// Output: 'Magento_Checkout/form/shipping'
```
...
defaults: {
    template: 'Vendor_Module/view/my-component'
}

// WRONG: Don't include .html
defaults: {
    template: 'Vendor_Module/view/my-component.html'  // WRONG
}

// Physical file location:
// app/code/Vendor/Module/view/frontend/web/template/view/my-component.html
//                                                   ^^^^^^^^^^^^^^^^^^^^
//                                                   This part matches the template string
```
...
> **Exam focus:** The `scope` binding is **Commerce-specific** (not native KO). It looks up a component by name in the `uiRegistry` and uses it as the binding context. This is how `checkout_index_index.xml` structure is rendered.
...
- [ ] `uiComponent` base class = `Magento_Ui/js/lib/core/collection`
...
- [ ] Checkout components live in `Magento_Checkout/js/view/`
- [ ] `quote.js` = singleton model with shared observables (shippingAddress, shippingMethod, etc.)
...
- [ ] `scope` binding = Commerce-specific, looks up component by name in `uiRegistry`
