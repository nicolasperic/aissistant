...
- Blocks **always** need a `class` attribute pointing to a valid PHP block class
...
- `ifExists="true"` prevents errors if the element doesn't exist in the current context
...
```
Core file location:
  app/code/Magento/Catalog/view/frontend/web/css/source/_module.less
...
|---|---|---|
| `<block>` | Creates a new block | Forgetting `class` attribute |
...
- [ ] `<block>` must have a `class` attribute pointing to a PHP class
