...
|---|---|---|
| `0` | `USER_TYPE_GUEST` | Anonymous / guest |
| `1` | `USER_TYPE_CUSTOMER` | Logged-in customer |
| `2` | `USER_TYPE_ADMIN` | Admin user |
| `3` | `USER_TYPE_INTEGRATION` | Integration token |
...
| `CouldNotSaveException` | 500 Internal Error | Database save failed |
| `CouldNotDeleteException` | 500 Internal Error | Delete failed |
| `AlreadyExistsException` | 409 Conflict | Duplicate entity |
| `LocalizedException` | 400 Bad Request | Generic user-facing error |
| `\Exception` | 500 Internal Error | Unexpected error |

**Exam focus:**
- Exception mapping to HTTP codes is handled by `Magento\Webapi\Controller\Rest\InputParamsResolver`
...
- [ ] `InputException::wasErrorAdded()` checks if any errors were collected
- [ ] `CouldNotSaveException` / `CouldNotDeleteException` → HTTP 500
