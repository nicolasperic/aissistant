...
**Q19.** What does the `db_schema_whitelist.json` (or `db_schema_allowlist.json` in 2.4.x) file contain?
...
```json
// app/code/Vendor/Module/etc/db_schema_allowlist.json
// (was db_schema_whitelist.json before 2.4.4)
{
    "vendor_custom_table": {
        "column": {
            "entity_id": true,
...
**Exam focus:** The allowlist/whitelist JSON must exist for destructive operations (DROP COLUMN, DROP TABLE) to work during module uninstall. Generate it, never write it by hand.
...
- [ ] `db_schema_allowlist.json` required for destructive operations
