...
| `vertex` | Vertex Tax Config | Vertex integration config |
...
> **Exam focus:** Search criteria parameters: `searchCriteria[filterGroups][0][filters][0][field]`, `[value]`, `[conditionType]`. Multiple filters in the same group are **AND** conditions. Multiple filter groups are **OR** conditions.
...
> **Exam focus — Trap:** "Multiple filters in the same `filterGroup` are OR conditions" → **Wrong**. Filters within the **same group** are **AND**. Different `filterGroups` are **OR**.
...
- [ ] Same filterGroup filters = **AND**; different filterGroups = **OR**
- [ ] Admin token endpoint: `POST /V1/integration/admin/token`
- [ ] Token in header: `Authorization: Bearer {token}`
- [ ] Async bulk endpoint: `/async/bulk/V1/` (requires RabbitMQ)
...
- [ ] Same filterGroup = OR → **WRONG** (same group = AND)
