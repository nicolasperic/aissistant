...
- The `revert()` method is required by the interface but only called in special rollback scenarios
...
| `Magento\Catalog\Model\Product\Attribute\Backend\Media` | Media gallery |

**Exam focus:**
- Backend models hook into `beforeSave`, `afterSave`, `beforeLoad`, `afterLoad`, `validate`
...
class MemberPrice extends RegularPrice
{
    ...
    public function getValue(): float|bool
    {
...
- Use the **pricing pool** (via `di.xml`) for clean price modifier injection rather than overriding price models
...
#### Step 3: Register Filter in `di.xml`
...
<config xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    ...
    <!-- Register in the FilterList -->
    <type name="Magento\Catalog\Model\Layer\FilterList">
        <arguments>
            <argument name="filters" xsi:type="array">
                <item name="fabric_type" xsi:type="array">
                    <item name="model" xsi:type="string">Vendor\Module\Model\Layer\Filter\FabricType</item>
                    <item name="attributeCode" xsi:type="string">fabric_type</item>
                </item>
            </argument>
        </arguments>
    </type>
...
- The `FilterList` is populated from `di.xml` — custom filters registered there
...
- [ ] Data Patches implement `DataPatchInterface` with `apply()`, `revert()`, `getDependencies()`, `getAliases()`
...
- [ ] Backend models hook: `beforeSave`, `afterSave`, `beforeLoad`, `afterLoad`, `validate`
...
- [ ] Preferred approach: add custom price to the **pricing pool** via `di.xml`
...
- [ ] Register custom filters in `Magento\Catalog\Model\Layer\FilterList` via `di.xml`
...
bin/magento eav:attributes:cleanup             # Remove unused EAV attribute values
```
