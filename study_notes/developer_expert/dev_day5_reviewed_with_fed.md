...
+---------------------------+
           ...
           v
+---------------------------+
|  CspWhitelistXmlCollector |  <-- aggregates all whitelist XML files
|  (+ other collectors)    |
+---------------------------+
           |
           v
+---------------------------+
|  HTTP Response Headers    |  <-- injected by Magento\Csp\Observer\Render
+---------------------------+
```

> **Note:** `Magento\Csp\Model\CompliancePolicy` does **not** exist in Commerce 2.4.x. The whitelist XML is read by `Model\Collector\CspWhitelistXmlCollector`. Header injection happens via the `Observer\Render` observer (which calls `CspRenderer`), **not** a plugin named `CspHeaderPlugin`.
    ...
    <policies>
        ...
        <policy id="script-src">
            <values>
                ...
                <value id="google-tag-manager" type="host">www.googletagmanager.com</value>
            </values>
        </policy>
        ...
        <policy id="style-src">
            <values>
                <value id="google-fonts-css" type="host">fonts.googleapis.com</value>
            </values>
        </policy>
        ...
        <policy id="connect-src">
            <values>
                <value id="my-api" type="host">api.example.com</value>
            </values>
        </policy>
...
> **Important:** Real Magento whitelist files (e.g. `module-paypal/etc/csp_whitelist.xml`) use **bare hostnames without a scheme prefix** — `www.paypal.com`, not `https://www.paypal.com`. Including `https://` is valid CSP syntax and restricts the allowance to HTTPS-only, but the Magento convention is scheme-less (allows both HTTP and HTTPS).
...
> **Note:** The Commerce default (`module-csp/etc/config.xml`) ships with **both** storefront and admin set to `report_only = 1` (report-only mode). The snippet above shows how to override storefront to enforcement mode while keeping admin in report-only.
...
> **Important:** The form key is stored in a **cookie** named `form_key` (managed by `module-page-cache/view/frontend/web/js/form-key-provider.js`). It is **not** a customer-data section in `localStorage`. Use `$.mage.cookies.get('form_key')` to read it in JavaScript — this is the pattern used throughout Commerce core (e.g. `checkout/js/sidebar.js`, `checkout/js/action/update-shopping-cart.js`).

```javascript
// Correct: read form_key from the cookie
...
```javascript
// Alternative: read from the hidden input field already on the page
var formKey = document.querySelector('input[name="form_key"]').value;
```

**Exam focus:**
- The form key is stored as a **cookie** (`form_key`) set by `form-key-provider.js` on page load
...
> **Important:** DOMPurify does **not** have a registered RequireJS alias in Commerce core (`domPurify` is not defined in any Magento `requirejs-config.js`). To use it in a custom module or theme, you must first bundle the library and define the alias:
>
> ```javascript
> // In your theme's requirejs-config.js
> var config = {
>     map: { '*': { 'domPurify': 'Vendor_Module/js/lib/purify.min' } }
> };
> ```
>
> Without this, `require(['domPurify'], ...)` silently fails (module not found).

```javascript
// After defining the alias in requirejs-config.js:
...
- No built-in RequireJS alias — must be set up manually in your module/theme
...
**Real `vendor/magento/module-paypal/etc/csp_whitelist.xml` (simplified):**
...
> Note: host values are bare hostnames — no `https://` prefix.
...
- [ ] Host values in real Magento whitelist files use bare hostnames (e.g. `www.paypal.com`), not `https://www.paypal.com`
...
- [ ] Default (from `module-csp/etc/config.xml`): both storefront and admin default to `report_only = 1`
- [ ] Nonces are generated per-request via `CspNonceProvider::generateNonce()` (`Magento\Csp\Helper\CspNonceProvider`)
- [ ] CSP headers are injected by `Magento\Csp\Observer\Render` (not a plugin)
...
- [ ] Form key is stored in a **cookie** (`form_key`) — NOT localStorage or customer-data
- [ ] AJAX: read form key via `$.mage.cookies.get('form_key')` (requires `mage/cookies` dependency)
...
- [ ] No built-in `domPurify` RequireJS alias in Commerce — must define it manually in `requirejs-config.js`
