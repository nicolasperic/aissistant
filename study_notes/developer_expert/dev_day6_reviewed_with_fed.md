...
<!-- CORRECTED: Index group settings differ significantly from default. Source: vendor/magento/module-indexer/etc/cron_groups.xml and vendor/magento/module-cron/etc/cron_groups.xml -->
...
> **Exam focus:** The `index` group uses much shorter scheduling windows (`schedule_generate_every=1`, `schedule_ahead_for=4`, `schedule_lifetime=2`) than the `default` group. This ensures index jobs are picked up quickly and expired jobs are marked missed sooner.
...
<!-- CORRECTED: status column is varchar(7) per db_schema.xml, not varchar(28) -->
...
-- | Field         | Type                | Null |
-- +---------------+---------------------+------+
-- | schedule_id   | int unsigned        | NO   | (PK, auto_increment)
...
<!-- CORRECTED: update/cron.php does not exist in Magento 2.4+. The second entry varies by setup. -->
<!-- In Magento 2.4+ on-premise, both entries are typically cron:run variants. The update/cron.php  -->
<!-- entry was part of the Component Manager in Magento 2.1-2.3 and has been removed.              -->
...
# Entry 2: Update/deploy process runner (Magento 2.1-2.3 only — uses update/cron.php)
# In Magento 2.4+ this entry is no longer needed. Some deployments add a second
# cron:run entry (e.g., targeting --group=index) for explicit group isolation.
...
<!-- CORRECTED: cron:status does not exist in Magento 2.4.8-p3. Only cron:run, cron:install, and cron:remove are available. -->
...
# NOTE: bin/magento cron:status does NOT exist in Magento 2.4.
# To inspect scheduled jobs, query the cron_schedule table directly.
```
...
<!-- CORRECTED: history_failure_lifetime default is 4320 min (72 hours), not 600 min. -->
<!-- Source: vendor/magento/module-cron/etc/cron_groups.xml and vendor/magento/module-indexer/etc/cron_groups.xml -->
...
> **Exam focus:** `history_failure_lifetime` is intentionally long (4320 min = 72 hours) so developers have ample time to inspect failures before they are purged.
...
- [ ] `index` group uses shorter windows: `schedule_generate_every=1`, `schedule_ahead_for=4`, `schedule_lifetime=2`
...
- [ ] Generate is throttled by `schedule_generate_every` (default 15 min for `default` group, 1 min for `index`)
...
- [ ] Entry 2: Varies by Magento version/edition/hosting — `update/cron.php` was Magento 2.1-2.3 only (removed in 2.4+)
...
- [ ] `schedule_lifetime` (default 15 min for `default` group, 2 min for `index`) determines how late a job can start before it becomes `missed`
...
- [ ] NOTE: `bin/magento cron:status` does NOT exist — use direct DB queries instead
