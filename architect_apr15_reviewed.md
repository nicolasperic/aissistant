...
<routes xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    ...
    <!-- /V1/customers/me maps to CustomerRepositoryInterface::getById()
         with customerId injected from the token via <data><parameter> -->
...
**Exam focus:**
- `ref="self"` is **not just documentation** — Magento enforces it at the framework level; a customer with token A **cannot** access another customer's data even if the ID exists.
...
- The **me** URL convention signals `ref="self"` — the framework injects the actual customer ID from the token via a `<data><parameter force="true">%customer_id%</parameter></data>` block.
...
```php
// Api/Data/ProductInterface.php — docblock annotations drive REST serialization
...
interface ProductInterface
{
    ...
    public function getExtensionAttributes(); // -> "extension_attributes": {}
}
```
...
| Customer Token | Bearer token via `/V1/integration/customer/token` | Configurable (default **1h**) | Customer-facing apps, mobile |
...
**Defaults confirmed in `vendor/magento/module-integration/etc/config.xml`:**
```xml
<access_token_lifetime>
    <customer>1</customer>   <!-- 1 hour -->
    <admin>4</admin>         <!-- 4 hours -->
</access_token_lifetime>
```
...
**Exam focus:**
- Admin token expiry is set in **Stores > Configuration > Services > OAuth > Access Token Expiration** (section `oauth`, group `access_token_lifetime`, field `admin`).
...
class CustomAttribute implements ResolverInterface
{
     ...
     *   In practice, Magento passes Magento\GraphQl\Model\Query\ContextInterface
     *   which extends the base empty ContextInterface and adds getUserId(), getUserType(),
     *   getExtensionAttributes() — cast to it for customer-aware logic.
...
**Important:** The `$context` parameter is declared as `$context` (untyped) in `ResolverInterface` — Magento passes a `Magento\GraphQl\Model\Query\Context` object that implements both the framework `ContextInterface` and the module-level `Magento\GraphQl\Model\Query\ContextInterface`. To call `getUserId()`, cast: `/** @var \Magento\GraphQl\Model\Query\ContextInterface $context */ $context->getUserId()`.
...
<config xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    ...
    <preference for="Vendor\Module\Api\Resolver\CustomAttributeInterface"
                type="Vendor\Module\Model\Resolver\CustomAttribute"/>
...
- `$context` in practice is `Magento\GraphQl\Model\Query\Context`; call `$context->getUserId()` for customer-aware resolvers (method is on `Magento\GraphQl\Model\Query\ContextInterface`, not the base empty framework interface).
...
class Seller implements BatchResolverInterface
{
    ...
    /**
     * @param ContextInterface $context  <-- FIRST parameter (required)
     * @param Field $field
...
```php
// vendor/magento/framework/GraphQl/Query/Resolver/BatchRequestItemInterface.php
interface BatchRequestItemInterface
{
    ...
    // NOTE: getContext() does NOT exist on BatchRequestItemInterface
    // Context is passed as the first parameter to BatchResolverInterface::resolve()
}
```

**Exam focus:**
- `BatchResolverInterface` replaces `ResolverInterface` — they are **mutually exclusive**; implement one or the other.
- **Critical:** `BatchResolverInterface::resolve()` signature is `resolve(ContextInterface $context, Field $field, array $requests): BatchResponse` — `$context` is the **first** parameter.
- The batch resolver receives an **array** of `BatchRequestItemInterface` — one per product that needs this field.
- `BatchRequestItemInterface` provides `getValue()`, `getArgs()`, `getInfo()` — there is NO `getContext()` method on it; context arrives via the `$context` parameter to `resolve()`.
...
```graphql
# Pattern 1: Interface usage with @typeResolver for polymorphism
interface ProductInterface @typeResolver(class: "Magento\\CatalogGraphQl\\Model\\ProductInterfaceTypeResolverComposite")
    @doc(description: "Contains fields that are common to all types of products.") {
    uid: ID! @doc(description: "The unique ID for a `ProductInterface` object.")
        @resolver(class: "Magento\\CatalogGraphQl\\Model\\Resolver\\Product\\EntityIdToUid")
...
# SimpleProduct implements multiple interfaces (not just ProductInterface)
...
**Correct answer:** Convert the brand resolver to implement `BatchResolverInterface`. Instead of one DB query per product, collect all `brand_id` values from all product requests and execute a single `WHERE id IN (...)` query, then distribute results via `BatchResponse`. Remember: `BatchResolverInterface::resolve()` takes `($context, $field, $requests)` — context is first.
...
- [ ] `/V1/customers/me` GET maps to `CustomerRepositoryInterface::getById()` with `customerId` injected from token.
...
- [ ] Admin tokens: simple Bearer, default **4h** expiry, not for production integrations.
- [ ] Customer tokens: default **1h** expiry.
...
- [ ] `ResolverInterface`: single-item resolvers, top-level queries. Signature: `resolve(Field $field, $context, ResolveInfo $info, ?array $value = null, ?array $args = null)`.
- [ ] `BatchResolverInterface`: field resolvers on list types — **prevents N+1 queries**. Signature: `resolve(ContextInterface $context, Field $field, array $requests): BatchResponse` — **context is FIRST**.
- [ ] `BatchRequestItemInterface` has: `getValue(): ?array`, `getArgs(): ?array`, `getInfo(): ResolveInfo` — **no `getContext()`** (context comes via `resolve()` parameter).
- [ ] `$value` in `resolve()` = parent resolver's output array.
- [ ] `$context->getUserId()` = current customer ID — method is on `Magento\GraphQl\Model\Query\ContextInterface`, not the base empty `ContextInterface`.
