...
// Return: ?array — null to leave args unchanged, array to replace args
// ⚠ Return type must be ?array (nullable), NOT : array
// Returning null = leave original arguments unchanged
// Returning array = replace arguments with the returned array
class BeforePlugin
{
    public function beforeSave(
        ...
        // Or: return null; // to leave arguments unchanged
    }
}
```
...
- Plugin on a **Plugin** is not possible — plugins cannot intercept other plugins (generated Interceptors implement `NoninterceptableInterface`)
...
- Repository pattern: `get()`, `getById()`, `getList()`, `save()`, `delete()`, `deleteById()` — these are the standard contract methods
...
#### CustomerRepositoryInterface — Actual Signatures

> **⚠ CORRECTION:** All `@api` interface methods have **NO PHP return/parameter type hints** — they use docblock `@return`/`@param` annotations only. The `save()` method also has an additional `$passwordHash` parameter. Confirmed from `vendor/magento/module-customer/Api/CustomerRepositoryInterface.php`.

```php
// Actual interface signatures (docblock types only, no PHP type hints):
interface CustomerRepositoryInterface
{
    // Note: save() has optional $passwordHash parameter
    public function save(\Magento\Customer\Api\Data\CustomerInterface $customer, $passwordHash = null);

    // @param string $email, @param int|null $websiteId
    ...
    // @param CustomerInterface $customer — @return bool
    public function delete(\Magento\Customer\Api\Data\CustomerInterface $customer);

    // @param int $customerId — @return bool
...
- Each `addFilter()` call creates a **new FilterGroup** → combined with **AND** between groups
- `addFilters([f1, f2])` puts multiple filters in the **same FilterGroup** → combined with **OR** within the group
...
| (Cache Tags)       |      e.g. ["cat_p_1", "cat_p_42"]
+--------------------+
     |
     v
...
```php
// Separate Identity class implementing IdentityInterface
// Namespace: Magento\Framework\GraphQl\Query\Resolver\IdentityInterface
// (vendor/magento/framework/GraphQl/Query/Resolver/IdentityInterface.php)
...
{
    // Product::CACHE_TAG = 'cat_p' (vendor/magento/module-catalog/Model/Product.php:65)
...
#### Registering the Identity Class — Schema Directive (NOT di.xml)

> **⚠ CORRECTION:** The identity class is registered via the **`@cache` directive in the schema.graphqls file**, NOT via a `di.xml` argument. There is no `identityClass` di.xml argument anywhere in the codebase. The `CacheableQueryHandler` reads the `cacheIdentity` key from the parsed schema annotation.

```graphql
# Correct way — specify identity class in schema.graphqls
type Query {
    products(
        search: String
        filter: ProductAttributeFilterInput
    ): Products
    @resolver(class: "Vendor\\Module\\Model\\Resolver\\ProductsResolver")
    @cache(cacheIdentity: "Vendor\\Module\\Model\\Resolver\\Cache\\ProductsIdentity")
}
```

Real core example from `module-catalog-graph-ql/etc/schema.graphqls`:
```
@cache(cacheIdentity: "Magento\\CatalogGraphQl\\Model\\Resolver\\Product\\Identity")
```

The `CacheableQueryHandler` extracts `cacheAnnotation['cacheIdentity']` from the parsed directive and instantiates the identity class via `IdentityPool::get()`.

**Exam focus:**
- GraphQL queries **can** be cached by Varnish/Fastly if they are `GET` requests — mutations are **never cached**
- The `X-Magento-Cache-Id` header (`CacheIdCalculator::CACHE_ID_HEADER`) drives cache variation for GraphQL (similar to `X-Magento-Vary` for page cache)
...
- Queries marked as `@cache(cacheable: false)` in the schema will not be cached regardless of headers — confirmed in multiple core schema files (quote, customer, wishlist, etc.)
- The `@cache` directive is defined in `module-graph-ql/etc/schema.graphqls`: `directive @cache(cacheIdentity: String="" cacheable: Boolean=true) on QUERY`
...
- Only one preference can win for a given interface/class — the **last module loaded** wins, where load order is determined by `sequence` declarations in `module.xml` (not alphabetical order)
...
"Real-time" data (cart, customer session, prices with rules):
  --> Should use @cache(cacheable: false) in schema
  --> Or rely on private content mechanisms, not FPC

Static/catalog data (product listings, CMS pages):
  --> Should be cached with proper IdentityInterface implementation
  --> Identity class specified via @cache(cacheIdentity: "...") in schema.graphqls
```
...
| **GraphQL caching** | New area of exam, less real-world exposure | IdentityInterface = cache tag registration via `@cache(cacheIdentity: "...")` in schema; mutations never cache |
...
- [ ] `before()` → modify input arguments → return `?array` (nullable) — null = unchanged, array = new args
...
- [ ] Plugins cannot intercept other plugins (Interceptors implement `NoninterceptableInterface`)
- [ ] Plugins cannot be applied to `final` classes or `final` methods
...
- [ ] Real `@api` interface methods have **no PHP type hints** — docblock annotations only
- [ ] Use `SearchCriteria` + `SearchCriteriaBuilder` for repository queries in business logic
- [ ] Multiple `addFilter()` calls on same builder = **AND** logic (each creates a new FilterGroup)
- [ ] `addFilters(array)` = **OR** logic within the array (same FilterGroup)
...
- [ ] Cache invalidation uses `IdentityInterface` → `getIdentities(array $resolvedData): array`
- [ ] Identity class is wired via `@cache(cacheIdentity: "ClassName")` directive **in schema.graphqls** — NOT via di.xml
- [ ] `X-Magento-Cache-Id` header (`CacheIdCalculator::CACHE_ID_HEADER`) drives GraphQL cache variation
...
- [ ] `@cache(cacheable: false)` in schema prevents caching regardless of headers
- [ ] `@cache` directive definition: `directive @cache(cacheIdentity: String="" cacheable: Boolean=true) on QUERY`
- [ ] `Product::CACHE_TAG = 'cat_p'` → cache tags format: `cat_p_42`
