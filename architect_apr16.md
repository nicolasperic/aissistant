...
sortOrder 20   ->  Standard Router    (frontend module routes)
sortOrder 60   ->  CMS Router         (cms_page URL keys)
sortOrder 100  ->  URL Rewrite Router (url_rewrite table)
sortOrder 200  ->  Default Router     (renders 404)
```

> **Exam focus:** The numbers above are approximate defaults — what matters is the *relative order* and *why* each sits where it does. The exam may describe a scenario and ask which router handles it.

### Why This Order?

```
Admin before Standard
  -> Admin routes (/admin/*) must be isolated from frontend
  -> Security: admin area must not accidentally fall through to frontend
...
CMS before URL Rewrite
  -> CMS pages are first-class content entities
  -> URL Rewrites can point to CMS pages, products, categories
  -> Prevents rewrite table from overriding a live CMS page

URL Rewrite before Default (404)
  -> Last chance to find something useful
  -> If nothing found: Default Router returns 404 page
```
...
- Renders the `noroute` action: `\Magento\Cms\Controller\Noroute\Index`
- Dispatches layout handle `default` + `cms_index_noroute`
            ...
            <argument name="routerList" xsi:type="array">
                <item name="vendor_brands" xsi:type="array">
                    ...
                    <!--
                        ...
                        Put before Standard (20) if you need priority
                        Put after Standard (20) to let explicit routes win first
                    -->
...
<config xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
        xsi:noNamespaceSchemaLocation="urn:magento:module-adobe-io-events-client:etc/io_events.xsd">
...
```php
<?php
// File: Vendor/Module/Plugin/OrderPlugin.php
// Using the I/O Events client to dispatch a custom event

use Magento\AdobeIoEventsClient\Api\EventPublisherInterface;
use Magento\AdobeIoEventsClient\Model\Data\AdobeIoEventFactory;

class OrderPlugin
{
    public function __construct(
        private EventPublisherInterface $eventPublisher,
        private AdobeIoEventFactory $eventFactory
    ) {}
    ...
    public function afterSave(
        ...
        $event->setName('vendor.module.order.completed');
        $event->setData([
...
- Requires `magento/module-adobe-io-events-client` extension
...
### Browsing `module-async-order`

```
vendor/magento/module-async-order/
  |
  +-- etc/
  |    +-- communication.xml    <- Topic: async.operations.all
  |    +-- queue.xml            <- Consumer: async.operations.all -> BulkOperationsConsumer
  |
  +-- Model/
       +-- BulkPlaceOrderOperation.php  <- Publisher side
       +-- OrderPlacement/
            +-- Order.php               <- Consumer handler
```

```xml
<!-- Simplified from module-async-order/etc/queue.xml -->
<config xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
        xsi:noNamespaceSchemaLocation="urn:magento:framework-message-queue:etc/queue.xsd">
    <broker topic="async.operations.all"
            type="db"
            connection="db">
        <queue name="async.operations.all"
               handler="Magento\AsynchronousOperations\Model\MassConsumer::process"
               consumerInstance="Magento\Framework\MessageQueue\Consumer"
               maxMessages="100"/>
    </broker>
</config>
...
### Browsing `module-inventory-message-bus`

```
vendor/magento/module-inventory-message-bus/
  |
  +-- etc/
  |    +-- communication.xml    <- 7+ inventory-related topics
  |    +-- queue.xml            <- Consumers for each topic
  |
  +-- Model/
       +-- GetReservationsQuantityFromQueue.php
       +-- Publisher/
            +-- ProductReservationsPublisher.php
```

```xml
<!-- Simplified from module-inventory-message-bus/etc/communication.xml -->
<config xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
        xsi:noNamespaceSchemaLocation="urn:magento:framework:Communication/etc/communication.xsd">

    <!-- Inventory reservation topic: fires when stock is reserved at checkout -->
    <topic name="inventory.reservations.updateSalabilityStatus"
           request="Magento\InventoryReservationsApi\Model\ReservationInterface[]">
    </topic>

    <!-- Stock index update topic: fires when stock quantity changes -->
    <topic name="inventory.indexer.sourceItem"
           request="Magento\InventoryApi\Api\Data\SourceItemInterface[]">
    </topic>

</config>
```

> **Exam focus:** Notice that `module-inventory-message-bus` topics send **arrays of objects** (e.g., `ReservationInterface[]`). This is batch processing — one message can contain multiple reservations to process together. This reduces queue overhead for high-order-volume stores.
...
### Router Priority (sortOrder)

- [ ] Admin (10) → Standard (20) → CMS (60) → URL Rewrite (100) → Default/404 (200)
- [ ] Standard Router: `frontName/controller/action` mapped via `routes.xml`
- [ ] CMS Router: database-driven, queries `cms_page` table
- [ ] URL Rewrite Router: queries `url_rewrite` table, supports 301/302
- [ ] Default Router: always matches, renders 404
...
- [ ] Registered in `io_events.xml` per module
